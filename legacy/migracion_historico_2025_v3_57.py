...
Ver CHANGELOG completo en docs/ESTADO_PROYECTO.md
"""
...
v3.57 - JIMELUZ REESCRITO (OCR + TABLA IVA) + FIX MADRUEÑO:
      - JIMELUZ: Nuevo extractor con doble estrategia:
        1. Extrae líneas individuales con patrón flexible (tolera OCR malo)
        2. Usa tabla IVA del ticket como fallback/validación
        3. Fallback a TOTAL FACTURA si OCR muy malo
      - Patrón mejorado: acepta cantidad como |, l, I (errores OCR típicos)
      - Si OCR malo pierde líneas, genera líneas desde tabla IVA
      - Validación: cuadra con TOTAL FACTURA del ticket
      - LICORES MADRUEÑO: Añadido patrón "TOTAL €:" + FALLBACK robusto
        con múltiples patrones alternativos para Windows
      - Probado con 4 facturas JIMELUZ: 100% detección

v3.56 - FIX SOM ENERGIA + SERRIN + GREDALES + GADITAUN:
      - Bug SOM ENERGIA: patrón IBARRAKO capturaba saltos de línea
        Fix: cambiar \\s+ por [ \\t]+ en patrón "XX,XX € YY,YY €"
      - Bug SERRIN NO CHAN: extraer_total capturaba base sin IVA
        Fix: nuevo patrón "XXX,XX €YYY,YY €TOTALES" prioritario
      - LOS GREDALES: Reescrito extractor para extraer líneas detalladas
        (cajas, botellas, precio, total) en vez de solo base imponible
      - GADITAUN / María Linarejos: Nuevo extractor OCR para patés gaditanos
        NIF: 34007216Z, IBAN: ES19 0081 0259 1000 0163 8268

v3.54 - SISTEMA HÍBRIDO PDF INTELIGENTE:
      - pypdf como extractor PRINCIPAL (compatibilidad con 58 extractores)
      - pdfplumber para proveedores problemáticos (CERES, etc.)
      - Diccionario EXTRACTOR_PDF_PROVEEDOR para control por proveedor
      - OCR Tesseract como último recurso
      - Preparación para campo precio_unitario

v3.53 - MIGRACIÓN A PDFPLUMBER + TESSERACT OCR:
      - pdfplumber como extractor principal (mejor para tablas)
      - Tesseract OCR como fallback para PDFs escaneados (imágenes)
      - Sistema dual: pdfplumber → si no hay texto → OCR
      - Nuevos extractores: JIMELUZ (OCR), LA ROSQUILLERIA (OCR)
      - Requisitos: pip install pdfplumber pytesseract pdf2image
      - Requisitos sistema: Tesseract-OCR, Poppler

v3.52 - NUEVOS EXTRACTORES (5 proveedores, 32 facturas):
      - EMJAMESA: reescrito con pdfplumber, soporta TARJETA y TRANSFERENCIA
      - CARRASCAL (EL CARRASCAL): nuevo extractor
      - SILVA CORDERO (Quesos Acehuche): nuevo extractor con portes
      - MOLIENDA VERDE: reescrito con pdfplumber, descripciones multilínea
      - ZUBELZU: reescrito con pdfplumber

v3.51 - ALQUILERES: soporte retención 19% IRPF
...
v3.49 - BERZAL y LAVAPIES: fix cuadre
      ...
      - LAVAPIES: usa bases declaradas del PDF en lugar de adivinar IVA

v3.41 - DISBER y ADELL: extractores corregidos/nuevos
      - DISBER: patron corregido para texto fragmentado por pypdf
      - ADELL: nuevo extractor (CANTIDAD+ARTICULO PRECIO IVA SUBTOTAL+LOTE)

v3.40 - FELISA GOURMET: corregido extractor
      - Patron actualizado: DESCRIPCION PRECIO IMPORTE+CODIGO(pegados) CANTIDAD
      - El codigo esta pegado al importe (34,00SAH120)
      - TRANSPORTE se ignora (se prorratea automaticamente)

v3.39 - CERES: SOUSAS y PLASTICO (formato sin descuento)
      - Nuevo patron para productos SIN columna descuento (SOUSAS, PLASTICO)
      - Formato: CODIGO DESC UDS IVA+PRECIO IMPORTE (sin DTO)
      - Corrige descuadres -10.44 EUR (SOUSAS) y -1.21 EUR (PLASTICO)

v3.37 - CERES ENVASES (devoluciones)
      - Extractor CERES: incluye lineas ENVASE (pueden ser negativas)
      - Corrige descuadres por devolucion de barriles (-150 EUR, etc.)

v3.36 - CERES CLA + DETECCIÓN PROVEEDOR POR CONTENIDO
      - Extractor CERES: añadida captura de CLA (caja retornable 1€, IVA 21%)
      - Nueva función detectar_proveedor_por_contenido() como fallback

v3.35 - FIX CERES SIN_TOTAL

v3.34 - REFACTOR: Constante VERSION única

v3.33 - MEJORA EXTRACCIÓN TOTAL (SIN_TOTAL)
...
# OCR libraries (opcional, para PDFs escaneados)
try:
    import pytesseract
    ...
    print("⚠️ OCR no disponible. Instalar con: pip install pytesseract pdf2image")
...
PROVEEDORES_CONOCIDOS = {
    ...
    # v3.56 - GADITAUN / María Linarejos
    ...
    # v3.56 - ECOMS SUPERMARKET / DIA
    'ECOMS': {'cif': 'B72738602', 'iban': ''},  # Pago tarjeta
    ...
    # v3.52 - EL CARRASCAL (Jose Luis Sánchez Martin)
    ...
    # v3.52 - SILVA CORDERO (Quesos de Acehuche)
    'SILVA CORDERO': {'cif': 'B09861535', 'iban': 'ES48 3001 0050 78 5010003340'},
    # v3.53 - JIMELUZ EMPRENDEDORES S.L. (EF - pago efectivo)
    'JIMELUZ': {'cif': 'B84527068', 'iban': ''},  # Pago efectivo
    'JIMELUZ EMPRENDEDORES': {'cif': 'B84527068', 'iban': ''},
    # v3.53 - LA ROSQUILLERIA S.L.U.
    ...
    # v3.55 - IBARRAKO PIPARRAK (guindillas)
    ...
    # v3.55 - MANIPULADOS ABELLAN (Productos El Labrador)
    ...
    # v3.36 - CERES (cervezas)
    'CERES': {'cif': 'B83478669', 'iban': ''},
    'CERES CERVEZA': {'cif': 'B83478669', 'iban': ''},
}

# =============================================================================
# v3.36 - DICCIONARIO INVERSO CIF → PROVEEDOR
# =============================================================================
# Para detectar proveedor por contenido del PDF cuando el nombre de archivo falla
...
CIF_A_PROVEEDOR = {
    ...
    '34007216Z': 'GADITAUN',  # v3.56 - María Linarejos
    'B72738602': 'ECOMS',  # v3.56 - ECOMS SUPERMARKET / DIA
    'B72113897': 'FELISA',
    ...
    'F11794542': 'SABORES DE PATERNA',
}


# =============================================================================
# v3.54 - EXTRACTOR PDF POR PROVEEDOR
# =============================================================================
# Por defecto se usa pypdf (compatibilidad con extractores existentes)
# Proveedores en esta lista usan pdfplumber (mejor para tablas/problemas cuadre)
# Valor 'ocr' fuerza OCR para proveedores con PDFs escaneados

EXTRACTOR_PDF_PROVEEDOR = {
    # Proveedores con problemas de cuadre → probar pdfplumber
    'CERES': 'pdfplumber',
    ...
    # Proveedores que ya funcionan bien con pdfplumber (v3.52)
    'EMJAMESA': 'pdfplumber',
...
def detectar_proveedor_por_contenido(texto: str) -> Optional[str]:
    """
    v3.36 - Detecta el proveedor buscando CIF conocido en el texto del PDF.
    
    Se usa como fallback cuando el nombre del archivo no identifica el proveedor.
    
    Args:
        texto: Texto extraído del PDF
        
    Returns:
        Nombre del proveedor si se encuentra CIF conocido, None en caso contrario
    """
    # Buscar cualquier CIF en el texto
    # Formatos: B12345678, 12345678X, ES-B12345678, etc.
    cifs_encontrados = re.findall(r'\b([A-Z]?\d{7,8}[A-Z]?)\b', texto)
    
    for cif in cifs_encontrados:
        # Normalizar CIF (quitar guiones, espacios)
        cif_limpio = cif.replace('-', '').replace(' ', '').upper()
        
        if cif_limpio in CIF_A_PROVEEDOR:
            return CIF_A_PROVEEDOR[cif_limpio]
    
    # También buscar formatos con letra al principio
    cifs_letra = re.findall(r'\b([A-Z]\d{8})\b', texto)
    for cif in cifs_letra:
        if cif in CIF_A_PROVEEDOR:
            return CIF_A_PROVEEDOR[cif]
...
class LineaFactura:
    ...
    codigo: str = ""
    retencion: int = 0  # v3.51: retención IRPF (alquileres)
    # v3.54: campos para histórico de precios
    cantidad: float = 0.0  # Cantidad comprada
    precio_unitario: float = 0.0  # Precio por unidad (sin IVA)
...
def extraer_texto_con_pypdf(ruta: Path) -> str:
...
def extraer_texto_con_pdfplumber(ruta: Path) -> str:
...
def extraer_texto_con_ocr(ruta: Path) -> str:
    """Extrae texto usando Tesseract OCR (para PDFs escaneados)."""
    texto = ""
    if not OCR_DISPONIBLE:
        return texto
    
    # v3.55 - Intentar primero con pdfplumber (mejor calidad de imagen)
        ...
        try:
            ...
            with pdfplumber.open(str(ruta)) as pdf:
                for page in pdf.pages:
                    ...
                    # OCR con configuración para tablas
                    try:
                        page_text = pytesseract.image_to_string(pil_img, lang='spa', config='--psm 6')
                    except:
            ...
            pass  # Fallback a pdf2image
    
    # Fallback: usar pdf2image (método original)
    try:
        images = convert_from_path(str(ruta), dpi=300)
        for img in images:
            try:
...
def extraer_texto_pdf(ruta: Path, proveedor: str = None) -> str:
    """
    v3.54 - Extrae texto de un PDF usando sistema inteligente por proveedor.
    
    Estrategia:
    1. Si proveedor está en EXTRACTOR_PDF_PROVEEDOR, usa el método indicado
    2. Si no, usa pypdf (compatibilidad con 58 extractores existentes)
    3. Si no hay texto, intenta fallbacks
    
    Args:
        ruta: Path al archivo PDF
        proveedor: Nombre del proveedor (opcional, para elegir extractor)
    
    Returns:
        str: Texto extraído del PDF
    """
    # Determinar método preferido según proveedor
    metodo_preferido = 'pypdf'  # Por defecto: compatibilidad
    
    if proveedor:
        proveedor_upper = proveedor.upper().strip()
        # Buscar en diccionario (coincidencia parcial)
        for prov_key, metodo in EXTRACTOR_PDF_PROVEEDOR.items():
            if prov_key in proveedor_upper or proveedor_upper in prov_key:
                metodo_preferido = metodo
                break
    
    texto = ""
    
    # PASO 1: Usar método preferido según proveedor
    if metodo_preferido == 'ocr':
        texto = extraer_texto_con_ocr(ruta)
        if texto.strip():
            return texto
        # Fallback a pypdf si OCR falla
        texto = extraer_texto_con_pypdf(ruta)
        if texto.strip():
            return texto
        texto = extraer_texto_con_pdfplumber(ruta)
        return texto
    
    elif metodo_preferido == 'pdfplumber':
        texto = extraer_texto_con_pdfplumber(ruta)
        if texto.strip():
            return texto
        # Fallback a pypdf
...
def extraer_total(texto: str) -> Optional[float]:
    """Extrae el total de la factura - v3.52 con soporte para más formatos."""
    ...
    # v3.41: Corregido patron 4 para no capturar "Subtotal"
    # v3.51: Añadido patrón para facturas con retención (alquileres)
    # v3.52: Añadido patrón para PC COMPONENTES (formato con punto decimal)
    # v3.54: Añadido patrón para CERES (Importe TOTAL ........ 697,23)
    # v3.54: Añadido patrón para BORBOTON (BASE € IVA% CUOTA € TOTAL €)
    # v3.54: Añadido patrón para ZUBELZU (TOTAL FACTURA en cabecera, valores en línea siguiente)
    # v3.54: Añadido patrón para LIDL (Total NETO IVA BRUTO - capturar BRUTO)
    # v3.54: Añadido patrón para MUÑOZ MARTIN (TOTAL\nBASE IVA% CUOTA TOTAL €)
    # v3.55: Añadido patrón para MANIPULADOS ABELLAN (tabla OCR)
    # v3.55: Añadido patrón para IBARRAKO PIPARRAK
    # v3.56: Añadido patrón para SERRIN NO CHAN y LOS GREDALES
    # v3.56: Reordenado BORBOTON antes de IBARRAKO para evitar falsos positivos
    patrones = [
        # v3.56 - Patrón LOS GREDALES: "IVA 21% 59,72 €\n12 72 344,12 €" → captura 344,12
        r'IVA\s*21%\s*[\d,]+\s*€\s*\d+\s+\d+\s+(\d{1,3}(?:[.,]\d{3})*[,\.]\d{2})\s*€',
        # v3.56 - Patrón SERRIN NO CHAN: "333,24 €389,54 €TOTALES" → captura 389,54
        r'[\d,]+\s*€(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€TOTALES',
        # v3.56 - Patrón BORBOTON (MOVIDO ARRIBA): "357,12 € 21,00 % 75,00 € 432,12 €"
        # Debe ir ANTES de IBARRAKO porque es más específico (requiere IVA%)
        r'(?:\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€\s+(?:21|10|4)[.,]00\s*%\s+(?:\d+[.,]\d{2})\s*€\s+(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€',
        # v3.56 - Patrón MARITA COSTA: "TOTAL: 643,82€"
        # Debe ir ANTES de IBARRAKO porque es más específico (requiere "TOTAL:")
        r'TOTAL:\s*([\d,]+)€',
        # v3.55 - Patrón IBARRAKO (formato antiguo): "396,00 39,60 1,40 0,00 Euros 435,60"
        r'Euros\s+(\d{1,3}[,\.]\d{2})',
        # v3.55 - Patrón IBARRAKO (formato nuevo): "168,00 € 184,80 €"
        # v3.56 - Fix: usar [ \t]+ en vez de \s+ para no capturar saltos de línea (problema Som Energia)
        r'[\d,]+\s*€[ \t]+(\d{1,3}[,\.]\d{2})\s*€\s*$',
        # v3.55 - Patrón EMJAMESA: total en línea sola después de desglose IVA
        # Formato: "21 20,40 € 21,000 % 4,28 €\n303,44 €"
        r'21,000\s*%\s*[\d,]+\s*€\n(\d{1,3}[,\.]\d{2})\s*€',
        # v3.55 - Patrón MANIPULADOS ABELLAN (OCR tabla): "552,36 552,36 10 55,24 607,60"
        # Formato: SUMA BASE IVA% IMP.IVA TOTAL → captura último (TOTAL)
        r'\d{3}[,\.]\d{2}\s+\d{3}[,\.]\d{2}\s+\d{1,2}\s+\d{2}[,\.]\d{2}\s+(\d{3}[,\.]\d{2})\s*$',
        # v3.54 - Patrón MUÑOZ MARTIN: "TOTAL\n112,26 21 23,57 135,83 €" -> último número
        r'IVA\s+TOTAL\n[\d,]+\s+\d+\s+[\d,]+\s+([\d,]+)\s*€',
        # v3.54 - Patrón LIDL: "Total 27,54 2,06 29,60" -> capturar el último (bruto)
        r'^Total\s+[\d,]+\s+[\d,]+\s+([\d,]+)\s*$',
        # v3.54 - Patrón ZUBELZU: "TOTAL FACTURA\nBRUTO BASE IVA% IMP.IVA TOTAL"
        # El último número de la línea es el TOTAL FACTURA
        r'TOTAL\s+FACTURA\n[\d,\.\s]+?(\d{1,3}(?:\.\d{3})*[,]\d{2})\s*$',
        # v3.54 - Patrón CERES: "Importe TOTAL ........ 697,23" o negativo "-399,30"
        r'Importe\s+TOTAL\s*[\.]+\s*(-?\d{1,3}(?:[.,]\d{3})*[.,]\d{2})',
        # v3.52 - Patrón PC COMPONENTES: TOTAL A PAGAR con punto decimal
        r'TOTAL\s*A\s*PAGAR\s+(\d+\.\d{2})\s*€',
        # v3.51 - Patrón ALQUILERES: TOTAL después de RETENCION (facturas con IRPF)
        # Formato: "RETENCION 19% -95,00 €\nTOTAL 510,00 €"
        r'RETENCION\s*\d+%?\s*-?\d+[.,]\d{2}\s*€?\s*\n?\s*TOTAL\s+(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€?',
        # v3.41 - Patrón MARTIN ABENZA - "IMPORTE IMPORTE XX,XX €" al FINAL
        # Formato: "288,00 IMPORTE IMPORTE 28,80 € IMPORTE IMPORTE 341,80 €"
        # DEBE IR PRIMERO porque aparecen varios "IMPORTE" y necesitamos el último
        r'IMPORTE\s+IMPORTE\s+(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€\s*$',
        # Patrón 1: TOTAL FACTURA / TOTAL IMPORTE / TOTAL FACT (muy específico)
        # v3.52: NO capturar si el número va seguido de % (sería porcentaje IVA)
        r'(?:TOTAL\s*FACTURA|TOTAL\s*IMPORTE|TOTAL\s*FACT|Total\s*Factura)[:\s]*(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})(?!\s*%)\s*€?',
        # v3.57 - Patrón LICORES MADRUEÑO: "TOTAL €: 984,58" o "TOTAL €: 1.020,45"
        ...
        # Patrón 4: TOTAL con € (word boundary para no capturar Subtotal)
        ...
        # Patrón 9: T O T A L (espaciado) - v3.41 añadido \b para evitar Subtotal
        r'\bT\s*O\s*T\s*A\s*L[:\s]*(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€?',
        # v3.29 - Patrón 10: Total XX,XX € (SEGURMA - "Total 48,23 €")
        r'\bTotal\s+(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€',
        # v3.33 - Patrones adicionales para más formatos
        # Patrón 11: Total con salto de línea antes del importe (pypdf a veces separa)
        r'\bTotal[\s\n]+(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})',
        # Patrón 12: "IMPORTE TOTAL" o "IMPORTE FACTURA"
        r'IMPORTE\s*(?:TOTAL|FACTURA)[:\s]*(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€?',
        # Patrón 13: "Total:" o "Total :" con variantes de espaciado
        r'\bTotal\s*:\s*(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})',
        # Patrón 14: Importe (genérico, al final del documento)
        r'Importe[:\s]+(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€',
        # Patrón 15: BASE + IVA = TOTAL en formato tabla (captura después de IVA)
        r'(?:Cuota\s*IVA|IVA\s*\d+%)[:\s]*\d+[.,]\d{2}[\s\n]+(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€?',
        # v3.41 - Patrón 16: ADELL - XX,XX € en linea sola ANTES de "TOTAL FACTURA"
        r'(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€\s*\n(?:Descuento|TOTAL FACTURA)',
        # v3.52 - Patrón 17: XX,XX Euros (MOLIENDA VERDE y otros)
        r'(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*Euros',
    ]
    
    for patron_str in patrones:
        patron = re.compile(patron_str, re.IGNORECASE | re.MULTILINE | re.DOTALL)
        match = patron.search(texto)
        if match:
            total_str = match.group(1)
            # v3.52: Detectar formato (punto o coma como decimal)
            # Si tiene punto y NO coma, es formato americano (90.83)
            ...
            try:
                valor = float(total_str)
                # Validación: el total debe ser razonable (> 1€ o < -1€ para devoluciones)
                if valor > 1.0 or valor < -1.0:
                    return valor
            except:
                continue
    
    # v3.52 - FALLBACK CARRASCAL: total ANTES de "TOTAL FACTURA" (solo si es factura CARRASCAL)
    if 'Jose Luis' in texto or 'CARRASCAL' in texto.upper() or '07951036M' in texto:
        ...
        match = patron_carrascal.search(texto)
        if match:
    ...
    # v3.54 - FALLBACK CERES: el total está después de "Importe TOTAL" pero con líneas intermedias
    # Formato pypdf: "Importe TOTAL ........\nNIF: B83478669\n700,46\nCLA:"
    if 'CERES' in texto.upper() or 'B83478669' in texto:
        # Buscar número justo antes de "CLA:"
        patron_ceres = re.compile(r'(\d+[.,]\d{2})\s*\nCLA:', re.MULTILINE)
        match = patron_ceres.search(texto)
        if match:
            total_str = match.group(1).replace(',', '.')
            try:
                return float(total_str)
    ...
    # v3.57 - FALLBACK LICORES MADRUEÑO: buscar TOTAL €: con variantes de espaciado
    # El PDF puede tener diferentes encodings en Windows vs Linux
    if 'MADRUEÑO' in texto.upper() or 'B86705126' in texto or 'B-86705126' in texto:
        # Patrón más flexible: TOTAL seguido de € y luego número
        patrones_madrueño = [
            r'TOTAL\s*€[:\s]*(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})',  # TOTAL €: 890,08
            r'TOTAL\s*€\s*(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})',     # TOTAL € 890,08
            r'(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€\s*\n.*?DATOS\s*BANCARIOS',  # 890,08 €\nDATOS BANCARIOS
            r'VENCIMIENTO[:\s]+\d+/\d+/\d+\s+(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€',  # VENCIMIENTO: fecha TOTAL €
        ]
        for patron_str in patrones_madrueño:
            match = re.search(patron_str, texto, re.IGNORECASE | re.MULTILINE)
            if match:
                total_str = match.group(1).replace('.', '').replace(',', '.')
                try:
                    valor = float(total_str)
                    if valor > 10:  # Mínimo razonable para MADRUEÑO
                        return valor
                except:
                    continue
    
    # v3.56 - FALLBACK ARGANZA: OCR no captura bien el total, calcularlo desde base
    # El PDF escaneado tiene mala calidad en la tabla de totales
    if 'ARGANZA' in texto.upper() or 'B24416869' in texto:
        # Buscar BASE IMPONIBLE en el texto OCR
        patron_base = re.search(r'BASE\s*IMPONIBLE.*?(\d{1,3}[,\.]\d{2})', texto, re.IGNORECASE)
        if not patron_base:
            # Intentar calcular suma de líneas
            patron_linea = re.compile(r'^[A-Z][A-Z0-9]{1,5}\s+.+?\s+(\d{1,3}[,\.]\d{2})$', re.MULTILINE)
            suma = 0
            for m in patron_linea.finditer(texto):
                suma += float(m.group(1).replace(',', '.'))
            if suma > 0:
                # IVA 21% para vinos
...
def validar_cuadre(lineas: List, total_factura: Optional[float], tolerancia: float = 0.05) -> str:
    ...
    v3.51 - Soporte para retención IRPF (facturas de alquiler).
    
    Args:
        lineas: Lista de líneas extraídas (cada una con 'base', 'iva' y opcionalmente 'retencion')
    ...
    # Calcular suma de bases + IVAs - retenciones
    suma_bases = 0.0
    suma_ivas = 0.0
    ...
    for linea in lineas:
        ...
        retencion_pct = linea.get('retencion', 0) or 0  # v3.51
        
        suma_bases += base
        suma_ivas += base * (iva_pct / 100.0)
        suma_retenciones += base * (retencion_pct / 100.0)  # v3.51
    
    # v3.51: Total = Base + IVA - Retención
    total_calculado = suma_bases + suma_ivas - suma_retenciones
...
def extraer_lineas_berzal(texto: str) -> List[Dict]:
    """Extrae líneas de facturas BERZAL HERMANOS S.A.
    
    v3.49 - Soporte dual pypdf/pdfplumber
    
    pypdf puede extraer con espacios internos en números:
    - "1 0" en lugar de "10" (IVA)
    - "1 8,90" en lugar de "18,90" (importe)
    - "3,1 50" en lugar de "3,150" (precio)
    
    Solución: preprocesar texto para eliminar UN espacio entre dígitos.
    """
    lineas = []
    
    # Preprocesar: eliminar UN espacio (no múltiples) entre dígitos o coma/punto
    # "1 0" -> "10", "1 8,90" -> "18,90", "3,1 50" -> "3,150"
    # Pero "10       5,48" se mantiene (múltiples espacios = separador real)
    texto_limpio = re.sub(r'(\d) (\d)', r'\1\2', texto)
    texto_limpio = re.sub(r'(\d) ([,\.])', r'\1\2', texto_limpio)
    texto_limpio = re.sub(r'([,\.]) (\d)', r'\1\2', texto_limpio)
    
    # Patrón 1: pypdf - CODIGO CONCEPTO IVA IMPORTE
    ...
    # Patrón 2: pdfplumber - CODIGO ... U/K IVA IMPORTE (al final de línea)
    ...
    # Intentar primero con pdfplumber (más específico) - usar texto original
    ...
    # Si no encontró nada, intentar con pypdf - usar texto limpio
    if not lineas:
...
def extraer_lineas_sabores_paterna(texto: str) -> List[Dict]:
    ...
    v3.54 - Añadida extracción de cantidad y precio_unitario
    ...
    # Patrón actualizado: captura unidades y peso para calcular cantidad
    patron = re.compile(
        r'(\d{2}-\d{2}-\d{2})'         # Fecha (dd-mm-yy)
        r'([A-Z][A-Z\s\.\d]+?)\s+'     # Descripción
        r'([\d,]+)\s+'                 # Unidades (capturar)
        r'([\d,]+)\s+'                 # Peso (capturar)
        r'(\d{1,3}[,\.]\d{2})'         # Precio
    ...
    for match in patron.finditer(texto):
        ...
        if len(descripcion_limpia) > 3:
            # v3.54 - Calcular cantidad (usar peso si es significativo, sino unidades)
            ...
            lineas.append({
                'codigo': '',
                'articulo': descripcion_limpia,
                'cantidad': cantidad,  # v3.54
                'precio_unitario': float(precio.replace(',', '.')),  # v3.54
                'iva': int(iva),
                'base': float(importe.replace(',', '.'))
            })
...
def extraer_lineas_francisco_guerra(texto: str) -> List[Dict]:
    ...
    v3.54 - Añadido cantidad y precio_unitario
...
def extraer_lineas_emjamesa(texto: str) -> List[Dict]:
    """Extrae líneas de facturas EMJAMESA S.L. (Ibéricos).
    
    v3.52 - Reescrito con pdfplumber
    v3.55 - Corregido: incluir portes para que se prorrateen
    
    Dos formatos:
    - TARJETA: sin portes, total después de cabecera
    - TRANSFERENCIA: con portes código 01, IVA 21%
    
    IBAN: ES08 3016 0206 52 2185032527
    """
    lineas = []
    ...
    # Patrón CON columna IVA al final
    ...
    if len(matches_con_iva) >= len(matches_sin_iva):
        for match in matches_con_iva:
            ...
            iva = int(iva_str)
            base = float(importe_str.replace(',', '.'))
            # v3.55: Incluir portes (código 01) - el prorrateo los procesará
            lineas.append({
                'codigo': codigo,
                'articulo': desc.strip(),
                'iva': iva,
        ...
        for match in matches_sin_iva:
            ...
            base = float(importe_str.replace(',', '.'))
            # v3.55: Incluir portes (código 01) - el prorrateo los procesará
            # Los portes van al 21%, el resto al 10%
...
def extraer_lineas_ceres(texto: str) -> List[Dict]:
    ...
    v3.54 - REESCRITO para formato pdfplumber (más limpio)
    
    Formato pdfplumber (columnas separadas):
    001124 MH CLASICA BARRIL 30L 6 108,8 50 21 326,40
    CE1408 ALH ESP 1/3 RET 24UDS 2 29,99 50 21 29,99
    001229 SOUSAS GAS MEDIO RT 20 UDS 1 9,49 10 9,49
    
    Columnas: CODIGO DESCRIPCION UDS PRECIO DTO IVA IMPORTE
    - Algunos productos no tienen DTO (SOUSAS, envases)
    - IVA puede ser 21 o 10
    - Importe puede ser negativo (devoluciones)
    """
    lineas = []
    codigos_procesados = set()
    
    # Patrón principal: CODIGO DESC UDS PRECIO DTO IVA IMPORTE
    # Ejemplo: 001124 MH CLASICA BARRIL 30L 6 108,8 50 21 326,40
    # Ejemplo con dto decimal: CE1384 VERMUT BENDITO BOX 20L 1 113,7 22,5 21 88,12
    patron_con_dto = re.compile(
        r'^([A-Z0-9]{6})\s+'           # Código (6 chars: 001124, CE1408)
        r'(.+?)\s+'                     # Descripción (flexible)
        r'(-?\d+)\s+'                   # Unidades
        r'(\d+[,\.]\d+)\s+'             # Precio/Ud
        r'(\d+[,\.]?\d*)\s+'            # Descuento % (puede tener decimal: 22,5)
        r'(21|10)\s+'                   # IVA (21 o 10)
        r'(-?\d+[,\.]\d+)',             # Importe
        re.MULTILINE
    )
    
    # Patrón sin descuento: CODIGO DESC UDS PRECIO IVA IMPORTE
    # Ejemplo: 001229 SOUSAS GAS MEDIO RT 20 UDS 1 9,49 10 9,49
    patron_sin_dto = re.compile(
        r'^([A-Z0-9]{6})\s+'           # Código
        r'(.+?)\s+'                     # Descripción
        r'(-?\d+)\s+'                   # Unidades
        r'(\d+[,\.]\d+)?\s*'            # Precio (opcional para regalos)
        r'(21|10)\s+'                   # IVA
        r'(-?\d+[,\.]\d+)',             # Importe
        re.MULTILINE
    )
    
    # Patrón envases litros: CE990001 ENVASE 50 lit. -11 30 21 -330,00
    ...
    # v3.56 - Patrón envases ALH: CE990013 ENVASE 1/5 ALH 6 2,7 21 16,20
    patron_envase_alh = re.compile(
        r'^(CE99\d{4})\s+'              # Código CE99xxxx
        r'(ENVASE\s+\d/\d\s+ALH)\s+'    # Descripción (ENVASE 1/5 ALH)
        r'(-?\d+)\s+'                   # Unidades
        r'(\d+[,\.]\d+)\s+'             # Precio (decimal)
        r'(21|10)\s+'                   # IVA
        r'(-?\d+[,\.]\d+)',             # Importe
        re.MULTILINE
    )
    
    # Patrón CE99 genérico (PLASTICO, etc): CE990020 PLASTICO 1/3 1 1 21 1,00
    patron_ce99_generico = re.compile(
        r'^(CE99\d{4})\s+'              # Código CE99xxxx (8 chars)
        r'(.+?)\s+'                     # Descripción
        r'(-?\d+)\s+'                   # Unidades
        r'(\d+)\s+'                     # Precio (entero)
        r'(21|10)\s+'                   # IVA
    ...
    for m in patron_con_dto.finditer(texto):
        ...
        # Ignorar si es albarán o cabecera
    ...
    # Extraer sin descuento (si no se capturó antes)
    for m in patron_sin_dto.finditer(texto):
        ...
        # Ignorar regalos (importe 0)
        if importe_val == 0:
            continue
            
        codigos_procesados.add(key)
        precio_ud = float(precio.replace(',', '.')) if precio else 0
        
        lineas.append({
            'codigo': codigo,
            'articulo': desc_limpia,
            'cantidad': abs(int(uds)),
            'precio_unitario': precio_ud,
            'iva': int(iva),
            'base': importe_val
        })
    ...
    # v3.56 - Extraer envases ALH (1/5, 1/3)
    ...
    # Extraer CE99 genérico (PLASTICO, etc) - si no se capturó antes
    for m in patron_ce99_generico.finditer(texto):
        codigo, desc, uds, precio, iva, importe = m.groups()
        desc_limpia = desc.strip()
        
        # Ignorar envases ya procesados
        if 'ENVASE' in desc_limpia.upper():
            continue
...
def extraer_lineas_molienda_verde(texto: str) -> List[Dict]:
    ...
    v3.54 - REESCRITO para pdfplumber
    
    Formato: CODIGO(3 dígitos) DESCRIPCION CANT PRECIO DTO IVA IMPORTE
    Las descripciones pueden ocupar 2 líneas en el PDF.
    
    Ejemplos pdfplumber:
    - Completa: "307 MERMELADA DE MORA A.E. 275 GR. 12 2,30 0 10 27,60"
    - Partida: "213 MERMELADA DE ARÁNDANOS con STEVIA 275 GR.Sin"
               "azúcar añadido 6 2,20 0 10 13,20"
    - Portes:  "PORTES 1 12,40 0 21 12,40"
    
    IBAN: ES41 3023 0407 1669 9576 7701
    """
    lineas = []
    lineas_texto = texto.split('\n')
    ...
    while i < len(lineas_texto):
        ...
        # Si empieza con código de 3 dígitos
        if re.match(r'^\d{3}\s+', linea):
            # Verificar si está completa (termina con patrón: CANT PRECIO DTO IVA IMPORTE)
            if re.search(r'\d+\s+\d+[.,]\d{2}\s+\d+\s+\d+\s+\d+[.,]\d{2}$', linea):
                lineas_unidas.append(linea)
            else:
                # Línea incompleta, unir con siguiente
                if i + 1 < len(lineas_texto):
                    siguiente = lineas_texto[i + 1].strip()
                    # La siguiente debe tener los números al final
                    if re.search(r'\d+\s+\d+[.,]\d{2}\s+\d+\s+\d+\s+\d+[.,]\d{2}$', siguiente):
                        linea_completa = linea + ' ' + siguiente
                        lineas_unidas.append(linea_completa)
                        i += 1
                    else:
                        lineas_unidas.append(linea)
                else:
    ...
    # CODIGO DESCRIPCION CANT PRECIO DTO IVA IMPORTE
    patron = re.compile(
        r'^(\d{3})\s+'                    # Código (3 dígitos)
        r'(.+?)\s+'                        # Descripción
        r'(\d+)\s+'                        # Cantidad
        r'(\d+[.,]\d{2})\s+'              # Precio
        r'(\d+)\s+'                        # Descuento
...
def extraer_lineas_felisa(texto: str) -> List[Dict]:
    """Extrae lineas de facturas FELISA GOURMET (PESCADOS DON FELIX).
    
    v3.54 - Reescrito: formato real es CODIGO DESCRIPCION CANTIDAD Unidades/Kilos PRECIO TOTAL
    
    Ejemplo texto extraido:
        SAH120 SARDINA AHUMADA 120GR OVAL 10 Unidades 3,4000 34,00
        TN MOJAMA 4UNID 1,005 Kilos 44,0000 44,22
    
    IMPORTANTE: TRANSPORTE se extrae como linea separada (IVA 21%).
    """
    lineas = []
    
    # Patron: CODIGO DESCRIPCION CANTIDAD Unidades/Kilos PRECIO TOTAL
    patron = re.compile(
        r'^([A-Z][A-Z0-9]*)\s+'       # Codigo (SAH120, TN, FSAH500)
        r'(.+?)\s+'                    # Descripcion
        r'([\d,]+)\s+'                 # Cantidad
        r'(?:Unidades|Kilos)\s+'       # Unidad
        r'([\d,]+)\s+'                 # Precio (4 decimales)
        r'([\d,]+)',                   # Total (2 decimales)
        re.MULTILINE
    )
    ...
    for match in patron.finditer(texto):
        ...
        # Ignorar lineas de albaran o lote
        if 'Albaran' in desc_limpia or 'Lote:' in desc_limpia or 'TOTAL' in desc_limpia:
            continue
        
        # Ignorar lineas con descripcion muy corta
        if len(desc_limpia) < 3:
            continue
        
        # Convertir cantidad (puede ser decimal para kilos)
        try:
    ...
    # TRANSPORTE se añade como linea para que el sistema de prorrateo lo distribuya
    # v3.54 - Formato real: "TRANSPORTE 8,30" o "TRANSPORTE\n8,30"
    if 'TRANSPORTE' in texto:
        # Formato 1: TRANSPORTE 8,30 (en la misma línea)
        match_transp = re.search(r'TRANSPORTE\s+([\d,]+)', texto)
        if match_transp:
            valor = float(match_transp.group(1).replace(',', '.'))
            if valor < 50:  # Validar que no sea el subtotal
                lineas.append({
                    'codigo': 'TRANSP',
                    'articulo': 'TRANSPORTE',
                    'cantidad': 1,
...
def extraer_lineas_borboton(texto: str) -> List[Dict]:
    ...
    # v3.54 - Corregido: soporta "Promocion 5+1", "Promoción 6+3", "Promocion especial", etc.
    patron_promo = re.compile(
        r'^Promoci[oó]n\s+(?:especial\s+)?(?:\d+\+\d+)?\s*'  # "Promoción X+Y" o "especial"
        r'(\d+)\s+'  # Unidades
...
def extraer_lineas_gredales(texto: str) -> List[Dict]:
    """Extrae líneas de facturas LOS GREDALES DE EL TOBOSO (vinos ecológicos).
    
    v3.56 - Extractor simplificado
    
    El PDF tiene formato inconsistente con datos dispersos, por lo que
    usamos BASE IMPONIBLE como método fiable.
    ...
    # Método principal: usar BASE IMPONIBLE
...
def extraer_lineas_gaditaun(texto: str) -> List[Dict]:
    """Extrae líneas de facturas GADITAUN / María Linarejos (patés y conservas gaditanas).
    
    v3.56 - Nuevo extractor OCR
    
    Formato Zoho CRM con tabla de IVAs por tipo (21%, 10%, 4%, 2%)
    Necesita OCR porque el PDF es imagen.
    ...
    # Buscar tabla de desglose por tipo IVA
    # Formato: "21% 44,25 € 9,29 €" o "10% 95,60 € 9,56 €"
    ...
    # Método alternativo: buscar Base Imponible total
    if not lineas:
        patron_base = re.search(r'Base\s*Imponible\s*([\d,]+)\s*€', texto, re.IGNORECASE)
        patron_total_iva = re.search(r'Total\s*IVA\s*([\d,]+)\s*€', texto, re.IGNORECASE)
        
        if patron_base:
            base = float(patron_base.group(1).replace(',', '.'))
            # Detectar IVA
            iva = 10  # Default para productos alimentarios
            if patron_total_iva:
                iva_importe = float(patron_total_iva.group(1).replace(',', '.'))
                # Calcular IVA aproximado
                if base > 0:
                    iva_calc = round(iva_importe / base * 100)
                    if iva_calc in [4, 10, 21]:
                        iva = iva_calc
...
def extraer_lineas_ecoms(texto: str) -> List[Dict]:
    """Extrae líneas de facturas ECOMS SUPERMARKET / DIA.
    
    v3.56 - Nuevo extractor
    
    Dos formatos:
    1. Tickets escaneados (OCR): "4,00% 4,39 0,18" en tabla DESGLOSE IVA
    2. Tickets digitales DIA: "A 4% 4,11 € 0,16 €" en tabla IVA
    
    CIF: B72738602
    Pago: Siempre tarjeta (sin IBAN)
    """
    lineas = []
    
    # MÉTODO 1: Formato OCR - buscar sección DESGLOSE/TIPO IVA
    # Importante: solo buscar DESPUÉS de "TIPO IVA" para evitar capturar productos
    ...
    if idx_desglose >= 0:
        # Buscar solo en la sección de desglose (hasta TOTALES)
        seccion = texto[idx_desglose:idx_desglose+400]
        
        # Patrón OCR: "4,00% BASE CUOTA" - base es número corto (1-2 dígitos antes de coma)
        ...
        for m in patron_ocr.finditer(seccion):
            ...
            # Limpiar OCR: reemplazar : y . por ,
            base_clean = re.sub(r'[:\.]', ',', base_raw)
            # Si hay más de una coma, quedarse con la última como decimal
            ...
            try:
                ...
                # Validar: base típica entre 0.10 y 500€
    ...
    # MÉTODO 2: Formato DIA digital - "A 4% BASE € CUOTA €"
    if not lineas:
    ...
    # MÉTODO 3: Fallback para OCR muy malo - usar línea TOTALES
    # Formato: "TOTALES 2,19 0,46" o "TOTALES 8,79 0,90"
    if not lineas:
        patron_totales = re.search(r'TOTALES\s+([\d,\.]+)\s+([\d,\.]+)', texto, re.IGNORECASE)
        if patron_totales:
            base_raw = patron_totales.group(1).replace(',', '.')
            try:
                base = float(base_raw)
                if 0.1 < base < 500:
                    # Calcular IVA aproximado desde cuota
                    cuota_raw = patron_totales.group(2).replace(',', '.')
                    cuota = float(cuota_raw)
                    if cuota > 0:
                        iva_calc = round(cuota / base * 100)
                        iva = 21 if iva_calc > 15 else (10 if iva_calc > 6 else 4)
                    else:
                        iva = 21  # Default
                    
                    lineas.append({
                        'codigo': 'ECOMS',
                        'articulo': 'COMPRA ECOMS/DIA',
                        'iva': iva,
                        'base': round(base, 2)
                    })
            except:
...
def extraer_lineas_arganza(texto: str) -> List[Dict]:
    ...
    v3.56 - Reescrito para soportar OCR
    
    Formato PDF texto: PRECIO DTO IMPORTE+CANTIDAD+CODIGO DESCRIPCION
    Formato OCR: CODIGO DESCRIPCION ... IMPORTE (al final de línea)
    ...
    # MÉTODO 1: Patrón OCR - código al inicio, importe al final
    # Formato: P069 LEGADO DE FARRO VERDEJO ... 150,00
    patron_ocr = re.compile(
        r'^([A-Z][A-Z0-9]{1,5})\s+(.+?)\s+(\d{1,3}[,\.]\d{2})$',
        re.MULTILINE
    )
    ...
    for match in patron_ocr.finditer(texto):
        ...
        # Limpiar descripción (quitar números intermedios del OCR)
        desc = re.sub(r'[\d,\.]+\s+[\d,\.]+\s+[\d,\.]+$', '', desc_raw).strip()
        desc = re.sub(r'\s+L\d+[A-Z]*$', '', desc)  # Quitar lote
        
        # Ignorar cabeceras
        if 'DESCRIPCION' in desc.upper() or 'CANTIDAD' in desc.upper():
            continue
        if codigo in ['IMPORTE', 'CODIGO', 'TOTAL']:
            continue
    ...
    # MÉTODO 2: Patrón texto original (si OCR no encontró nada)
    if not lineas:
        ...
        for match in patron_texto.finditer(texto):
            precio, importe, cantidad, codigo, desc = match.groups()
            desc_limpia = re.sub(r'\s+L\d+[A-Z]*$', '', desc.strip())
...
def extraer_lineas_lidl(texto: str) -> List[Dict]:
    """Extrae líneas de facturas LIDL SUPERMERCADOS.
    
    v3.54 - Nuevo extractor
    
    CIF: A60195278
    
    Formato líneas:
        BOLSA DE RAFIA 1 Cant. 0,5400 0,54 21,00 0,11 0,65
        CALABACÍN 2,248 Kg 1,6815 3,78 4,00 0,15 3,93
        LIMÓN 1 KG 1 Cant. 2,6800 2,68 4,00 0,11 2,79
    
    Columnas: DESCRIPCION CANTIDAD UNIDAD PRECIO_UNIT IMPORTE_NETO IVA% CUOTA_IVA IMPORTE
    
    IVA: Explícito en cada línea (4%, 10%, 21%)
    """
    lineas = []
    
    # Patrón mejorado: descripción flexible (puede tener números como "LIMÓN 1 KG")
    patron = re.compile(
        r'^(.+?)\s+'                              # Descripción (cualquier cosa, no-greedy)
        r'([\d,]+)\s+'                            # Cantidad
        r'(Cant\.|Kg)\s+'                         # Unidad
        r'([\d,]+)\s+'                            # Precio unitario
        r'([\d,]+)\s+'                            # Importe neto (BASE)
        r'([\d,]+)\s+'                            # IVA %
        r'[\d,]+\s+'                              # Cuota IVA (ignorar)
        r'[\d,]+\s*$',                            # Importe bruto (ignorar)
        re.MULTILINE
    )
    
    for match in patron.finditer(texto):
        desc, cantidad, unidad, precio, base, iva_pct = match.groups()
        desc_limpia = desc.strip()
        
        # Ignorar cabeceras y líneas de totales
        if any(x in desc_limpia for x in ['Descripción', 'DESCRIPCION', 'Total', 'Tipo IVA', 'Neto']):
            continue
        
        # Convertir valores
        try:
            cant = float(cantidad.replace(',', '.'))
            precio_unit = float(precio.replace(',', '.'))
            importe = float(base.replace(',', '.'))
...
def extraer_lineas_lavapies(texto: str) -> List[Dict]:
    ...
    v3.49 - Usa bases declaradas del PDF para evitar errores de asignación de IVA
    
    CIF: F88424072
    IBAN: ES39 3035 0376 14 3760011213
    
    El PDF declara las bases por tipo de IVA:
    BASE IMP. AL 21% 62,40 IVA 21% 13,10
    BASE IMP. AL 10% 65,70 IVA 10% 6,57
    
    Usamos estas bases en lugar de adivinar el IVA de cada producto.
    """
    lineas = []
    
    # Extraer bases declaradas del PDF
    base_21 = None
    base_10 = None
    
    m21 = re.search(r'BASE IMP\.\s*AL\s*21%\s+([\d,\.]+)', texto)
    m10 = re.search(r'BASE IMP\.\s*AL\s*10%\s+([\d,\.]+)', texto)
    
    if m21:
        base_21 = float(m21.group(1).replace(',', '.'))
    if m10:
        base_10 = float(m10.group(1).replace(',', '.'))
    
    # Si encontramos bases declaradas, usarlas directamente
    if base_21 is not None or base_10 is not None:
        if base_21 and base_21 > 0:
            lineas.append({
                'codigo': 'BASE21',
                'articulo': 'Productos IVA 21%',
                'iva': 21,
                'base': base_21
            })
        if base_10 and base_10 > 0:
            lineas.append({
                'codigo': 'BASE10',
                'articulo': 'Productos IVA 10%',
                'iva': 10,
                'base': base_10
            })
        return lineas
    
    # Fallback: extraer productos individuales si no hay bases declaradas
    patron = re.compile(
    ...
    for match in patron.finditer(texto):
        ...
        # IVA por defecto 10% (alimentación)
        lineas.append({
            'codigo': codigo,
            'articulo': desc_limpia,
            'iva': 10,
...
def extraer_lineas_fabeiro(texto: str) -> List[Dict]:
    ...
    if not lineas:
        ...
        for m in patron_v323.finditer(texto):
            codigo, cantidad, concepto, iva, importe, precio = m.groups()
            concepto_limpio = re.sub(r'\s+', ' ', concepto.strip())
...
def extraer_lineas_serrin(texto: str) -> List[Dict]:
    ...
    # Extraer códigos con cantidad (incluye decimales como "0,860" o "1,155")
    # v3.56 - Fix: añadir soporte para cantidades decimales
    patron_codigo_cant = re.compile(r'([A-Z]{3,5})\s+(\d{1,3}(?:[,\.]\d{1,3})?)(?=[A-Z\n]|$)')
    
    codigos_cant = []
    for m in patron_codigo_cant.finditer(texto):
        codigo = m.group(1)
        cant_str = m.group(2).replace(',', '.')
        cantidad = float(cant_str) if '.' in cant_str else int(cant_str)
        if codigo not in ['TOTAL', 'IVA', 'IBAN', 'SOBRE']:
            codigos_cant.append({'codigo': codigo, 'cantidad': cantidad})
    
    # Correlacionar por posición
    # Si hay desajuste en códigos, usar solo artículos + datos numéricos
    n_min = min(len(articulos_iva), len(datos_numericos))
    n_codigos = len(codigos_cant)
    
    for i in range(n_min):
        codigo = codigos_cant[i]['codigo'] if i < n_codigos else 'SERRIN'
        cantidad = codigos_cant[i]['cantidad'] if i < n_codigos else 1
        lineas.append({
            'codigo': codigo,
...
def extraer_lineas_alquiler_ortega(texto: str) -> List[Dict]:
    ...
    v3.51 - Soporte retención 19% IRPF
    ...
    Fórmula: Total = Base + IVA(21%) - Retención(19%)
    Categoría fija: ALQUILER
    """
    lineas = []
    
    # Buscar base imponible en SUBTOTAL (más fiable)
    ...
    if match_sub:
        ...
        # Fallback: buscar en línea de alquiler
        ...
        if match:
            ...
            base = 500.00  # Valor por defecto conocido
    
    lineas.append({
        'codigo': 'ALQ',
        'articulo': 'Alquiler local Calle Rodas 2',
        'iva': 21,
        'retencion': 19,  # v3.51: retención IRPF
        'base': base,
        'categoria': 'ALQUILER'
    })
...
def extraer_lineas_alquiler_fernandez(texto: str) -> List[Dict]:
    ...
    Total = Base + IVA(21%) - Retención(19%)
    Categoría fija: ALQUILER
    """
    lineas = []
...
def extraer_lineas_munoz_martin(texto: str) -> List[Dict]:
    """Extrae líneas de facturas BODEGAS MUÑOZ MARTÍN - v3.54 actualizado.
    ...
    Formato línea (pdfplumber):
    076 R CAJA 6 BOT. VIÑA JESUSA TINTO 8 12,40 99,20 €
    COD DESCRIPCION CANT PRECIO IMPORTE €
    
    Formato alternativo (pypdf - columnas desordenadas):
    ...
    # Patrón 1 (pdfplumber): COD DESC CANT PRECIO IMPORTE €
    ...
    # Patrón 2 (pypdf): COD PRECIO IMPORTE € CANT DESCRIPCION
    ...
    # Intentar patrón nuevo primero
    matches = list(patron_nuevo.finditer(texto))
    
    if matches:
        for m in matches:
            cod, desc, cant, precio, importe = m.groups()
            desc_clean = desc.strip()
        ...
        # Fallback a patrón viejo
        for m in patron_viejo.finditer(texto):
...
def extraer_lineas_disber(texto: str) -> List[Dict]:
    """Extrae lineas de facturas DISBER (Grupo Disber) - v3.41.
    
    CIF: B46144424
    Productos: Vinos (21%), conservas gourmet (10%)
    IBAN: ES39 2100 8617 1502 0002 4610
    
    Formato en PDF visual:
    [CODIGO] Descripcion... CANTIDAD PRECIO_BASE PRECIO_UNIT IMPORTE
    
    Pero pypdf extrae fragmentado, necesitamos buscar el total desde el desglose fiscal.
    """
    lineas = []
    ...
    # Buscar productos por codigo entre corchetes
    # Ejemplo: [2VSME] Botella Vino Blanco HUELLA de Merseguera VEGAMAR
    ...
    for match in patron_producto.finditer(texto):
        ...
        # Quitar numeros sueltos y simbolos euro del final
        ...
        lineas.append({
            ...
            'articulo': desc[:60],
            'iva': iva,
            'base': 0  # Se calculara desde el total
        })
    
    # Obtener base imponible del desglose fiscal
    # Formato: "118,80 € IVA 21%" o "42,30 € IVA 10%"
    match_base = re.search(r'(\d+,\d{2})\s*€?\s*IVA\s*\d+%', texto)
    if match_base and lineas:
        base_total = float(match_base.group(1).replace(',', '.'))
        # Distribuir entre productos (simplificado: dividir equitativamente)
        base_por_producto = round(base_total / len(lineas), 2)
        for linea in lineas:
            linea['base'] = base_por_producto
        # Ajustar ultimo para que sume exactamente
        if lineas:
...
def extraer_lineas_zubelzu(texto: str) -> List[Dict]:
    """Extrae líneas de facturas ZUBELZU PIPARRAK S.L.
    
    v3.52 - Reescrito con pdfplumber
    
    Formato: Código | Concepto | Cajas | Cantidad | Precio | Total(BASE)
    - IVA siempre 10%
    - Sin portes
    - Total de línea es BASE (sin IVA)
    
    IBAN: ES61 3035 0141 8214 1001 9635
    """
    lineas = []
    
    # Patrón: Código | Descripción | Cajas | Cantidad | Precio | Total(BASE)
    patron_linea = re.compile(
        r'^(\d+)\s+(.+?)\s+(\d+)\s+(\d+)\s+([\d]+[,.][\d]{2,3})\s+([\d.,]+)$',
        re.MULTILINE
    )
    
    for match in patron_linea.finditer(texto):
        codigo, desc, cajas, cantidad, precio_str, total_str = match.groups()
        
        # El total viene como BASE (sin IVA)
        total_base = float(total_str.replace('.', '').replace(',', '.'))
...
def extraer_lineas_productos_adell(texto: str) -> List[Dict]:
    """Extrae lineas de facturas PRODUCTOS ADELL / Croquellanas - v3.41.
    
    CIF: B12711636
    Productos: Conservas gourmet (alcachofas, pates, mousses)
    IVA: 10%
    
    Formato linea real extraido por pypdf:
    24,000ALCACHOFAS EN ACEITE 330 GR 4,920 10,00 118,08240130 - 30/01/2026
    
    PROBLEMA: El subtotal (118,08) esta PEGADO al lote (240130)
    SOLUCION: Capturar exactamente 2 decimales para el subtotal
    """
    lineas = []
    
    # Patron corregido: CANTIDAD + DESCRIPCION + PRECIO + IVA + SUBTOTAL(2dec) + LOTE
    # El subtotal tiene exactamente 2 decimales, seguido inmediatamente por 6 digitos del lote
    patron = re.compile(
        r'(\d+[,.]\d{3})'                         # Cantidad (ej: 24,000 - exactamente 3 decimales)
        r'([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑ\s\d]+?)\s+'     # Descripcion (mayusculas)
        r'(\d+[,.]\d{3})\s+'                      # Precio (3 decimales)
        r'(\d+[,.]\d{2})\s+'                      # IVA (2 decimales: 10,00)
        r'(\d+[,.]\d{2})'                         # Subtotal (exactamente 2 decimales)
        r'(\d{6})\s*-\s*\d{2}/\d{2}/\d{4}',      # Lote (6 digitos) - Caducidad
        re.MULTILINE
    )
    
    for match in patron.finditer(texto):
        cantidad, descripcion, precio, iva_str, subtotal, lote = match.groups()
        ...
        lineas.append({
            ...
            'precio_unitario': float(precio.replace(',', '.')),
            'iva': int(float(iva_str.replace(',', '.'))),
            'base': round(float(subtotal.replace(',', '.')), 2),  # Base SIN IVA
            'categoria': ''  # Se asignara del diccionario
        })
...
# NOTA: extraer_lineas_ibarrako_piparrak() fue eliminada en v3.55
# Reemplazada por extraer_lineas_ibarrako() que es más robusta
...
def extraer_lineas_martin_abenza(texto: str) -> List[Dict]:
    """Extrae líneas de facturas MARTIN ABENZA / El Modesto - v3.41.
    
    NIF: 74305431K
    Productos: Conservas artesanas (lotes conservas variadas)
    IVA: 10%
    Categoría fija: CONSERVAS VEGETALES
    
    PROBLEMA: pypdf no extrae bien las columnas de este PDF.
    SOLUCIÓN: Extraer del resumen fiscal:
    - Total Bruto = Base imponible productos
    - IVA 10% = IVA de productos
    - Porte = Total - Total_Bruto - IVA (sin IVA)
    - Total = Total_Bruto + IVA + Porte
    
    Formato fiscal extraído:
    '288,00 IMPORTE IMPORTE 28,80 € IMPORTE IMPORTE 341,80 €'
    """
    lineas = []
    
    # Buscar resumen fiscal: TOTAL_BRUTO IMPORTE IMPORTE IVA € IMPORTE IMPORTE TOTAL €
    match = re.search(
        r'(\d+[,.]\d{2})\s+IMPORTE\s+IMPORTE\s+(\d+[,.]\d{2})\s*€\s+IMPORTE\s+IMPORTE\s+(\d+[,.]\d{2})\s*€',
        texto
    )
    
    if match:
        total_bruto = float(match.group(1).replace(',', '.'))
        iva_importe = float(match.group(2).replace(',', '.'))
        total = float(match.group(3).replace(',', '.'))
        
        # Calcular porte (sin IVA)
        porte = round(total - total_bruto - iva_importe, 2)
        
        # Línea 1: Productos con IVA 10%
        lineas.append({
            'codigo': '',
            'articulo': 'LOTES CONSERVAS VARIADAS',
            'cantidad': 1,
            'precio_unitario': total_bruto,
            'iva': 10,
            'base': total_bruto,
            'categoria': 'CONSERVAS VEGETALES'
        })
        
        # Línea 2: Porte sin IVA (si existe)
        if porte > 0:
            lineas.append({
                'codigo': '',
                'articulo': 'PORTE',
                'cantidad': 1,
                'precio_unitario': porte,
                'iva': 0,  # El porte no tiene IVA en este proveedor
                'base': porte,
                'categoria': 'PORTES'
            })
...
def extraer_lineas_pc_componentes(texto: str) -> List[Dict]:
    """Extrae líneas de facturas PC COMPONENTES.
    
    v3.52 - Nuevo extractor
    
    Formato: Código | Artículo | Precio | Und | Total
    - IVA 21% (electrónica)
    - Incluye descuentos (negativos) y portes
    - Total = BASE (ya sin IVA)
    
    CIF: B73347494
    """
    lineas = []
    
    # Patrón: Código Artículo Precio Und Total
    patron_linea = re.compile(
        r'^(\d+)\s+(.+?)\s+([-\d.]+)\s+(\d+)\s+([-\d.]+)$',
        re.MULTILINE
    )
    ...
    for match in patron_linea.finditer(texto):
        codigo, desc, precio_str, uds, total_str = match.groups()
        
        # Ignorar cabecera
        if 'Artículo' in desc or 'Código' in desc:
            continue
        
        total = float(total_str)
        
        # Los totales ya son BASE (el PDF indica Base Imponible)
        lineas.append({
            'codigo': codigo,
            'articulo': desc.strip()[:50],
            'cantidad': int(uds),
...
def extraer_lineas_carrascal(texto: str) -> List[Dict]:
    """Extrae líneas de facturas EL CARRASCAL (Jose Luis Sánchez Martin).
    
    v3.52 - Nuevo extractor
    
    Formato: Cantidad | Código(5 dígitos) | Artículo | Precio | IVA | Subtotal(BASE)
    - IVA 10%
    - Sin portes
    - Subtotal es BASE (sin IVA)
    
    IBAN: ES59 0049 0344 98 2510368354
    """
    lineas = []
    
    # Patrón: Cantidad | Código | Artículo | Precio | IVA | Subtotal(BASE)
    patron_linea = re.compile(
        r'^([\d,]+)\s+(\d{5})\s+(.+?)\s+([\d,]+)\s+([\d,]+)\s+([\d,]+)$',
        re.MULTILINE
    )
    
    for match in patron_linea.finditer(texto):
        cantidad_str, codigo, desc, precio_str, iva_str, subtotal_str = match.groups()
        
        cantidad = float(cantidad_str.replace(',', '.'))
        if cantidad == 0:  # Saltar líneas de LOTE
            continue
            
        precio_base = float(precio_str.replace(',', '.'))
        iva = int(float(iva_str.replace(',', '.')))
        subtotal_base = float(subtotal_str.replace(',', '.'))
        ...
        lineas.append({
            ...
            'iva': iva,
            'base': round(subtotal_base, 2)
        })
    
    return lineas


def extraer_lineas_silva_cordero(texto: str) -> List[Dict]:
    """Extrae líneas de facturas SILVA CORDERO (Quesos de Acehuche).
    
    v3.52 - Nuevo extractor
    
    Formato: Código+Descripción | Lote(6 dígitos) | Fec.Caduc | Cajas | Piezas | Cantidad | Precio€/kg | Dto | Importe(BASE)
    - IVA productos: 4% (quesos) o 2% según factura
    - Portes IVA 21% - se reparten proporcionalmente
    
    IBAN: ES48 3001 0050 78 5010003340
    """
    lineas = []
    
    # Patrón flexible para líneas de producto
    patron_linea = re.compile(
        r'^(.+?)\s+(\d{6})\s+(\d{2}/\d{2}/\d{2})\s+(\d+)\s+(\d+)\s+([\d,]+)\s+([\d,]+)€/kg\.\s+([\d,]+)\s+([\d,]+)$',
        re.MULTILINE
    )
    
    for match in patron_linea.finditer(texto):
        codigo_desc, lote, fec, cajas, piezas, cantidad_str, precio_str, dto_str, importe_str = match.groups()
        
        # Separar código de descripción
        partes = codigo_desc.strip().split()
        if len(partes) >= 2:
            if partes[0] in ['D.O.P', 'MINI'] or partes[0].isdigit() or re.match(r'^[A-Z0-9]+$', partes[0]):
                if len(partes) > 1 and partes[1] in ['DOP', 'D.O.P']:
        ...
        cantidad = float(cantidad_str.replace(',', '.'))
        precio_base = float(precio_str.replace(',', '.'))
        importe_base = float(importe_str.replace(',', '.'))
        
        lineas.append({
            'codigo': codigo,
            'articulo': descripcion.strip()[:50] if descripcion else codigo_desc.strip()[:50],
            'cantidad': cantidad,
            'precio_unitario': precio_base,
            'iva': 4,  # Quesos reducido, se ajusta abajo
            'base': round(importe_base, 2)
        })
    
    # Detectar IVA real de productos desde zona totales
    iva_pattern = re.findall(r'([\d,]+)\s+([\d,]+)\s+([\d,]+)\s*\n', texto)
    for base_str, iva_str, cuota_str in iva_pattern:
        base = float(base_str.replace(',', '.'))
        iva = float(iva_str.replace(',', '.'))
        if base > 50 and iva < 21:  # Base de productos, no portes
            for linea in lineas:
                linea['iva'] = int(iva)
            break
...
def extraer_lineas_jimeluz(texto: str) -> List[Dict]:
    """
    v3.57 - Extractor para JIMELUZ EMPRENDEDORES S.L. (OCR de tickets escaneados)
    
    Doble estrategia para máxima fiabilidad:
    1. Extrae líneas individuales con patrón flexible (tolera errores OCR)
    2. Usa tabla IVA del ticket como fallback cuando OCR es muy malo
    
    Formato ticket:
        CANT. DESCRIPCION           % IVA    IMPORTE
        1 LIMON CDC                 4,00     1,52
        2 HIELO CUBITOS 2 KG       10,00     2,50
                        TOTAL COMPRA          6,39
        
        4,00%    3,60    0,14    3,74    <- Tabla IVA
        10,00%   2,27    0,23    2,50
        21,00%   0,12    0,03    0,15
    """
    lineas = []
    
    # === MÉTODO 1: Extraer líneas individuales de artículos ===
    
    # Buscar zona de artículos (entre cabecera y TOTAL COMPRA)
    ...
    # Patrón flexible v3 - tolera errores OCR típicos:
    # - Cantidad puede ser 1, |, l, I (OCR confunde)
    # - IVA siempre es 4, 10 o 21 (los únicos válidos en España)
    # - Importe puede tener coma extra al inicio por OCR malo
    patron_linea = re.compile(
        r'^[1|lI\d]{0,2}\s*'                 # Cantidad (0-2 chars, puede faltar)
        r'(.+?)\s+'                          # Descripción (lazy match)
        r'(4|10|21)[,\.]\d{2}\.?\s*'         # IVA: solo 4, 10 o 21
        r',?(\d{1,3}[,\.]\d{1,2})',          # Importe (puede empezar con coma)
        re.MULTILINE
    )
    ...
    for match in patron_linea.finditer(zona_articulos):
        ...
        # Limpiar descripción (quitar ruido OCR)
        desc_limpia = re.sub(r'[£\*\'\"\|]+', '', desc).strip()
        desc_limpia = re.sub(r'\s+', ' ', desc_limpia)  # Espacios múltiples
        ...
        try:
            ...
            # El importe del ticket es CON IVA, calcular base
    ...
    # Si no hay líneas o son pocas, usar tabla IVA del ticket
    # Formato: 4,00%  BASE  CUOTA  [ruido]  TOTAL
    # El OCR a veces inserta dígitos sueltos entre CUOTA y TOTAL
    
    tabla_iva = {}
    patron_tabla = re.compile(
        r'(4|10|21)[,\.]\d{2}%?\s+'          # IVA: 4,00% o 10,00% o 21,00%
        r'\(?(\d{1,3}[,\.]\d{2})\)?\s+'      # BASE (puede tener paréntesis por OCR)
        r'\(?(\d{1,3}[,\.]\d{2})\)?\s+'      # CUOTA
        r'(?:\d\s+)?'                         # RUIDO OCR OPCIONAL (dígito suelto)
        r'\(?(\d{1,3}[,\.]\d{2})\)?'         # TOTAL
    )
    ...
    # Si no hay líneas pero sí tabla IVA, crear líneas genéricas
    ...
    # Si hay líneas pero la suma no cuadra con tabla IVA, ajustar
        ...
        # Si hay discrepancia > 5%, usar tabla IVA (más fiable que OCR de líneas)
    ...
    # === MÉTODO 3: Fallback a TOTAL FACTURA cuando OCR es muy malo ===
    # Si no hay líneas ni tabla IVA, buscar TOTAL FACTURA y crear línea genérica
    # Esto permite que al menos cuadre, aunque sin desglose
    if not lineas:
        ...
        if total_match:
            ...
            # Asumir IVA mixto (predominante en JIMELUZ: frutas 4%, hielo 10%, bolsas 21%)
            # Usar 10% como IVA promedio aproximado
            if total > 0 and total < 200:  # Filtrar valores absurdos del OCR
                base_estimada = total / 1.10  # Asumir ~10% IVA promedio
                lineas.append({
                    'codigo': '',
...
def extraer_lineas_rosquilleria(texto: str) -> List[Dict]:
    """
    v3.55 - Extractor para LA ROSQUILLERIA S.L.U. (OCR)
    
    Formato OCR:
        RN-1.15 ROSQUILLA 3825 6 15 90 1,02€ 4% 91,80
                ORIGINAL
        GSE     GASTOS DE ENVIO 1 1 10,00€ 0% 10,00
    
    El OCR puede perder la coma del precio: "102€" en vez de "1,02€"
    
    Los GASTOS DE ENVIO se mantienen como línea (0% IVA) para prorrateo.
    """
    lineas = []
    
    # Patrón para ROSQUILLA: permite precio sin coma (102€ en vez de 1,02€)
    # Busca: REF + ROSQUILLA + ... + PRECIO€ + IVA% + IMPORTE
    patron_rosquilla = re.compile(
        r'(RN[-\s]?\d+\.?\d*)\s+'                # Referencia (RN-1.15)
        r'(ROSQUILLA[A-Z\s]*?)\s+'               # Descripción
        r'.*?'                                    # Campos intermedios (lote, cajas, bolsas)
        r'(\d{1,3}[,\.]?\d{0,2})\s*€\s+'         # Precio (con o sin coma)
        r'(\d{1,2})%\s*'                          # IVA
        r'(\d{1,3}[,\.]\d{2})',                   # Importe final
        re.IGNORECASE | re.MULTILINE
    )
    
    # Patrón para GASTOS DE ENVIO
    patron_gse = re.compile(
        r'(GSE\s*\d?)\s+'                        # Referencia (GSE, GSE 2)
        r'(GASTOS?\s+DE\s+ENVIO[A-Z\s]*?)\s+'   # Descripción
        r'.*?'                                    # Campos intermedios
        r'(\d{1,3}[,\.]\d{2})\s*€\s+'            # Precio
        r'(\d{1,2})%\s*'                          # IVA (0%)
        r'(\d{1,3}[,\.]\d{2})',                   # Importe
        re.IGNORECASE | re.MULTILINE
    )
    ...
    for match in patron_rosquilla.finditer(texto):
        ref, desc, precio, iva, importe = match.groups()
        try:
            base = float(importe.replace(',', '.'))
            iva_int = int(iva)
            lineas.append({
    ...
    for match in patron_gse.finditer(texto):
        ref, desc, precio, iva, importe = match.groups()
        try:
            base = float(importe.replace(',', '.'))
            lineas.append({
...
def extraer_lineas_manipulados_abellan(texto: str) -> List[Dict]:
    """
    v3.55 - Extractor para MANIPULADOS ABELLAN / Productos El Labrador (OCR)
    
    Formato OCR (tabla con columnas CANTIDAD - CONCEPTO - PRECIO - IMPORTE):
        24,00 TOMATE ASADO LEÑA 720 ML EL LABRADOR 1,9500 46,80
        12,00 TOMATE RALLADO ESPECIAL TOSTADAS 720 ML 1,8500 22,20
    
    El OCR puede:
    - Convertir "24,00" en "2400" (sin coma)
    - Añadir caracteres al final: "66,601" en vez de "66,60"
    - Añadir ruido: "| 3 A", "77", etc.
    
    Todos los productos son IVA 10%.
    """
    lineas = []
    
    for line in texto.split('\n'):
        # Ignorar líneas de LOTE/CONSUMIR
        if 'LOTE' in line.upper() or 'CONSUMIR' in line.upper():
            continue
        
        # Extraer todos los números (incluye números sin coma como 2400)
        numeros = re.findall(r'\d{1,4}[,\.]?\d{0,4}', line)
        numeros = [n for n in numeros if len(n) >= 2]  # Filtrar números muy cortos
        if len(numeros) < 2:
            continue
        ...
        # Buscar cantidad:
        # - Formato normal: termina en ,00 o .00 (12,00, 24,00, 36,00, 48,00, 60,00, 108,00)
        # - Formato OCR malo: 4 dígitos que podrían ser cantidad*100 (1200, 2400, 3600, 4800, 6000, 10800)
        for num in numeros:
            if ',' in num or '.' in num:
                if num.endswith(',00') or num.endswith('.00'):
                    val = float(num.replace(',', '.'))
                    if val < 200 and val == int(val):
                        cantidad = val
                        break
            elif len(num) >= 4 and num.isdigit():
                # Posible cantidad sin coma: 2400 → 24.00, 10800 → 108.00
                val = int(num) / 100
                if val < 200 and val == int(val) and int(val) % 6 == 0:
                    cantidad = val
                    break
        
        # Buscar precio unitario: 4 decimales (1,9500, 2,1500, 3,5000)
        for num in numeros:
            if ',' in num or '.' in num:
                parts = num.replace('.', ',').split(',')
                if len(parts) == 2 and len(parts[1]) == 4:
                    precio = float(num.replace(',', '.'))
                    break
        
        # Buscar importe: calcular esperado y buscar coincidencia
        if cantidad and precio:
            esperado = cantidad * precio
            # Primero buscar con coma/punto
            for num in numeros:
                if ',' in num or '.' in num:
                    parts = num.replace('.', ',').split(',')
                    if len(parts) == 2 and len(parts[1]) >= 2:
                        # Tomar solo 2 decimales (por si OCR añadió más: 66,601 → 66,60)
                        val_str = parts[0] + '.' + parts[1][:2]
                        try:
                            val = float(val_str)
                            if abs(val - esperado) < 2.0:  # Tolerancia OCR
                                importe = val
                                break
                        except:
                            continue
            
            # v3.56 - Si no encontramos importe con coma, buscar sin coma (7740 → 77.40)
            if not importe:
                for num in numeros:
        ...
        if cantidad and precio and importe:
            # Extraer descripción
            match = re.search(r'(?:\d{1,5}[,\.]?\d{0,2})\s+([A-ZÁÉÍÓÚÑMO][A-Za-záéíóúñ\s\-\']+)', line)
            if match:
                desc = match.group(1).strip()
                desc = re.sub(r'[^\w\s]', '', desc)
                desc = re.sub(r'\s+', ' ', desc)
            else:
                desc = "PRODUCTO ABELLAN"
            
            # Corregir OCR: OMATE → TOMATE
            if desc.upper().startswith('OMATE'):
                desc = 'T' + desc
...
def extraer_lineas_ibarrako(texto: str) -> List[Dict]:
    """
    v3.55 - Extractor para IBARRAKO PIPARRAK S. Coop. (Guindillas)
    
    CIF: F20532297
    IBAN: ES69 2095 5081 9010 6181 7077 (KUTXABANK)
    
    Formato productos:
        2008 GALON IBARLUR PET 8,00 21,00 0,00 168,00
        2004 BOTES GALON 1ª 12,00 12,00 0,00 144,00
    ...
    # Patrón: CÓDIGO (4 dígitos) + DESCRIPCIÓN (puede incluir números como "1ª") + CANTIDAD + PRECIO + %DTO + IMPORTE
    patron = re.compile(
        r'^(\d{4})\s+([A-Za-z0-9áéíóúñÁÉÍÓÚÑª\s]+?)\s+(\d+[,\.]\d{2})\s+(\d+[,\.]\d{2})\s+(\d+[,\.]\d{2})\s+(\d+[,\.]\d{2})$',
        re.MULTILINE
    )
    
    for match in patron.finditer(texto):
        codigo, desc, cantidad_str, precio_str, dto_str, importe_str = match.groups()
        
        # Ignorar líneas de lote (empiezan con L seguido de números)
        if re.match(r'^L\d+$', desc.strip()):
            continue
...
def buscar_categoria(proveedor: str, articulo: str, indice: Dict, umbral: float = 0.8) -> Tuple[str, int]:
    ...
    # v3.52: Protección contra valores no-string
    if not isinstance(articulo, str):
        articulo = str(articulo) if articulo is not None else ''
    ...
    for item in articulos_prov:
        # v3.52: Protección contra valores no-string en diccionario
        item_articulo = item.get('articulo', '')
        if not isinstance(item_articulo, str):
            item_articulo = str(item_articulo) if item_articulo is not None else ''
        item_upper = item_articulo.upper()
...
def procesar_factura(ruta: Path, indice: Dict, carpeta_yaml: Path = None) -> Factura:
    ...
    # v3.54 - Extraer texto pasando proveedor para selección inteligente de extractor
    texto = extraer_texto_pdf(ruta, info['proveedor'])
    ...
    # v3.36 - Si el proveedor del archivo está vacío o es genérico, intentar detectar por contenido
    proveedor_detectado = info['proveedor']
    if not proveedor_detectado or len(proveedor_detectado) < 3:
        proveedor_por_cif = detectar_proveedor_por_contenido(texto)
        if proveedor_por_cif:
            proveedor_detectado = proveedor_por_cif
            info['proveedor'] = proveedor_por_cif
    ...
    # v3.27 - LA MOLIENDA VERDE (antes del genérico)
        ...
        # v3.35 - Extraer total específico para CERES
        # pypdf extrae: "Importe TOTAL ........\nNIF: B83478669\n588,06\nCLA:"
        # El valor está en línea separada después de "Importe TOTAL"
    ...
    elif 'GREDALES' in proveedor_upper or 'B83594150' in texto:
        lineas_raw = extraer_lineas_gredales(texto)
    # v3.56 - GADITAUN / María Linarejos (patés y conservas gaditanas)
    elif 'GADITAUN' in proveedor_upper or 'MARILINA' in proveedor_upper or 'LINAREJOS' in proveedor_upper or '34007216Z' in texto:
        lineas_raw = extraer_lineas_gaditaun(texto)
    # v3.56 - ECOMS SUPERMARKET / DIA (tickets supermercado)
    elif 'ECOMS' in proveedor_upper or 'DIA' in proveedor_upper or 'B72738602' in texto:
        lineas_raw = extraer_lineas_ecoms(texto)
    ...
    # v3.54 - LIDL SUPERMERCADOS
    elif 'LIDL' in proveedor_upper or 'A60195278' in texto:
        lineas_raw = extraer_lineas_lidl(texto)
    ...
    # v3.11 - MRM → YA DEFINIDO ARRIBA (línea ~6787)
    ...
    # v3.13 - PRODUCTOS ADELL / Croquellanas
    elif 'ADELL' in proveedor_upper or 'CROQUELLANAS' in proveedor_upper or 'B12711636' in texto or 'B-12711636' in texto:
    ...
    # v3.13 - IBARRAKO PIPARRAK → MOVIDO a v3.55 (extraer_lineas_ibarrako)
    # v3.13 - ANA CABALLO VERMOUTH
    ...
    # v3.15 - PANRUJE → YA DEFINIDO ARRIBA (línea ~6813)
    ...
    # v3.52 - EL CARRASCAL (Jose Luis Sánchez Martin)
    elif 'CARRASCAL' in proveedor_upper or 'JOSE LUIS SANCHEZ' in proveedor_upper or '07951036M' in texto:
        lineas_raw = extraer_lineas_carrascal(texto)
    # v3.52 - SILVA CORDERO (Quesos de Acehuche)
    elif 'SILVA CORDERO' in proveedor_upper or 'ACEHUCHE' in proveedor_upper or 'B09861535' in texto:
        lineas_raw = extraer_lineas_silva_cordero(texto)
        # Detectar y repartir portes
        portes_match = re.search(r'PORTES\s+([\d,]+)', texto)
        if portes_match and lineas_raw and factura.total:
            base_portes = float(portes_match.group(1).replace(',', '.'))
            if base_portes > 0:
                lineas_raw = repartir_portes(lineas_raw, factura.total, base_portes, iva_portes=21)
    # v3.52 - PC COMPONENTES
    elif 'PC COMPONENTES' in proveedor_upper or 'B73347494' in texto:
        lineas_raw = extraer_lineas_pc_componentes(texto)
    # v3.53 - JIMELUZ (OCR - tickets escaneados)
    elif 'JIMELUZ' in proveedor_upper or 'B84527068' in texto or 'EMBAJADORES' in texto.upper():
        lineas_raw = extraer_lineas_jimeluz(texto)
    # v3.53 - LA ROSQUILLERIA (OCR)
    # v3.55 - GASTOS DE ENVIO ya incluidos como línea, prorrateo automático más abajo
    elif 'ROSQUILLERIA' in proveedor_upper or 'B73814949' in texto:
        lineas_raw = extraer_lineas_rosquilleria(texto)
    # v3.55 - MANIPULADOS ABELLAN / El Labrador (OCR)
    elif 'ABELLAN' in proveedor_upper or 'MANIPULADOS' in proveedor_upper or 'LABRADOR' in proveedor_upper or 'B30473326' in texto:
        lineas_raw = extraer_lineas_manipulados_abellan(texto)
    # v3.55 - IBARRAKO PIPARRAK (guindillas vascas)
    elif 'IBARRAKO' in proveedor_upper or 'PIPARRAK' in proveedor_upper or 'PIPARRA' in proveedor_upper or 'F20532297' in texto:
        lineas_raw = extraer_lineas_ibarrako(texto)
    ...
    for linea in lineas_raw:
        ...
        factura.lineas.append(LineaFactura(
            ...
            iva=linea['iva'],
            categoria=categoria,
            retencion=linea.get('retencion', 0)  # v3.51
        ))
    ...
    # v3.21 - Validación de cuadre
    lineas_dict = [{'base': l.base, 'iva': l.iva, 'retencion': getattr(l, 'retencion', 0)} for l in factura.lineas]
...
def generar_excel_historico(facturas: List[Factura], ruta: Path):
    """Genera el Excel del histórico. v3.54: añade CANTIDAD y PRECIO_UD."""
    filas = []
        ...
        if f.lineas:
            for linea in f.lineas:
                filas.append({
                    ...
                    'ARTICULO': linea.articulo,
                    'CATEGORIA': linea.categoria,
                    'CANTIDAD': linea.cantidad if linea.cantidad else '',  # v3.54
                    'PRECIO_UD': linea.precio_unitario if linea.precio_unitario else '',  # v3.54
                    'TIPO IVA': linea.iva,
                    'BASE (€)': linea.base,
...
def generar_log(facturas: List[Factura], ruta: Path):
    ...
    with open(ruta, 'w', encoding='utf-8') as f:
        f.write(f"MIGRACIÓN HISTÓRICO 2025 - v{VERSION}\n")
        f.write(f"{'='*60}\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description=f'Migración histórico 2025 v{VERSION}')
    ...
    print("="*60)
    print(f"MIGRACIÓN HISTÓRICO 2025 - v{VERSION}")
    print("="*60)
    ...
    if len(facturas) > 0:
        ...
        print(f"  Con líneas: 0 (sin facturas)")
