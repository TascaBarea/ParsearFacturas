...
PROVEEDORES_CONOCIDOS = {
    ...
    # v3.17 - PORVAZ VILAGARCÍA / Conservas Tito
...
def extraer_fecha(texto: str, yaml_config: Optional[Dict] = None, proveedor: str = '') -> Optional[str]:
    ...
    # v3.17 - DISTRIBUCIONES LAVAPIES: "DD/MM/YY NNNNNN" al inicio
    ...
    # v3.17 - QUESOS FELIX / ARMANDO SANZ: "DD/MM/YYYY NN NNNNN"
    # Formato en PDF: 27/03/2025 25 00148 (fecha + año corto + número)
    if 'FELIX' in proveedor_upper or 'ARMANDO SANZ' in proveedor_upper or 'B47440136' in texto or 'B 47.440.136' in texto:
        # Buscar fecha DD/MM/YYYY seguida de "NN NNNNN" (año corto + número factura)
        patron = re.search(r'(\d{2})/(\d{2})/(\d{4})\s+\d{2}\s+\d{5}', texto)
        if patron:
            d, m, y = patron.groups()
            return f"{d}-{m}-{y[-2:]}"
        # Alternativa: formato con 6 dígitos
        patron2 = re.search(r'(\d{2})/(\d{2})/(\d{4})\s+\d{2}\s+\d{6}', texto)
        if patron2:
            d, m, y = patron2.groups()
            return f"{d}-{m}-{y[-2:]}"
    
    # v3.17 - CARLOS NAVAS / QUESOS NAVAS
    ...
    # v3.17 - SOM ENERGIA: "Fecha de la factura: DD/MM/YYYY"
...
def extraer_ref(texto: str, yaml_config: Optional[Dict] = None, proveedor: str = '') -> Optional[str]:
    ...
    # v3.17 - PORVAZ: "Número: 0454/25"
    if 'PORVAZ' in proveedor_upper or 'TITO' in proveedor_upper or 'B36281087' in texto:
        patron = re.search(r'N[úu]mero:\s*(\d+/\d+)', texto)
        if patron:
            return patron.group(1)
    
    # v3.17 - QUESOS FELIX: "FECHA    NUMERO" + newline + "DD/MM/YYYY  24 000720"
    # Formato: año (2 dígitos) + espacio + número (5-6 dígitos)
    # Ejemplo: "27/03/2025 25 00148" -> REF: "25-00148"
    if 'FELIX' in proveedor_upper or 'ARMANDO SANZ' in proveedor_upper or 'B47440136' in texto or 'B 47.440.136' in texto:
        patron = re.search(r'\d{2}/\d{2}/\d{4}\s+(\d{2})\s+(\d{5,6})', texto)
        if patron:
            año, num = patron.groups()
            return f"{año}-{num}"  # Formato: "25-00148"
    
    # v3.17 - CARLOS NAVAS / QUESOS NAVAS: "Nº Factura A/598"
    ...
    # v3.17 - SOM ENERGIA: "N.º de factura:FE2501314182"
    if 'SOM ENERGIA' in proveedor_upper or 'F55091367' in texto:
        patron = re.search(r'N\.º de factura:\s*(FE\d+)', texto)
        if patron:
            return patron.group(1)
    
    # v3.17 - ZUCCA / FORMAGGIARTE: "Factura 1 000407" -> "1-000407"
    # Formato: DOCUMENTO NÚMERO -> Factura [serie] [número]
    if 'ZUCCA' in proveedor_upper or 'FORMAGGIARTE' in proveedor_upper or 'B42861948' in texto:
    ...
    # v3.17 - BM SUPERMERCADOS: "Datos FACTURA: 142312502C0002320"
    if 'BM' in proveedor_upper or 'DISTRIBUCION SUPERMERCADOS' in texto or 'B20099586' in texto:
        patron = re.search(r'Datos FACTURA:\s*(\w+)', texto)
        if patron:
            return patron.group(1)
    
    # v3.17 - DISTRIBUCIONES LAVAPIES: "Nº DOCUMENTO" + fecha + número
    # Formato: "04/12/25 251131"
    if 'LAVAPIES' in proveedor_upper or 'F88424072' in texto:
        patron = re.search(r'Nº DOCUMENTO.*?(\d{2}/\d{2}/\d{2})\s+(\d{5,6})', texto, re.DOTALL)
        if patron:
            return patron.group(2)  # Solo el número
...
def extraer_lineas_porvaz(texto: str) -> List[Dict]:
    """Extrae líneas de facturas PORVAZ VILAGARCÍA / Conservas Tito - v3.17.
...
def extraer_lineas_quesos_felix(texto: str) -> List[Dict]:
    """Extrae líneas de facturas QUESOS FELIX / Armando Sanz - v3.17.
    ...
    Formato real del PDF (columnas concatenadas sin espacios claros):
    - QUESO FELIX. Especial PATAMULO GRANDE 3 5,250 15,00 78,75 €
    4175 QUESO IGP FELIX. VIEJO GRANDE 1 3,000 16,50 49,50 €
    ...
    # El € puede estar o no, separado por espacio o pegado
    patron = re.compile(
        r'^[\-\d]{1,5}\s+'                      # Lote: "-" o "4175"
        r'(QUESO[^\d]+?)\s+'                    # Descripción (hasta encontrar números)
        r'(\d+)\s+'                              # UDS (unidades enteras)
        r'(\d+[,\.]\d{3})\s+'                   # Cantidad (kg con 3 decimales)
        r'(\d+[,\.]\d{2})\s+'                   # Precio
        r'(\d+[,\.]\d{2})\s*€?',                # Importe (€ opcional)
        re.MULTILINE
    )
...
def extraer_lineas_carlos_navas(texto: str) -> List[Dict]:
    """Extrae líneas de facturas CARLOS NAVAS / QUESOS NAVAS - v3.17.
...
def buscar_categoria(proveedor: str, articulo: str, indice: Dict, umbral: float = 0.8) -> Tuple[str, int]:
    ...
    # v3.17: Verificar si hay alias para este proveedor
    ...
    # v3.17: Limpiar artículo antes de buscar
...
def procesar_factura(ruta: Path, indice: Dict, carpeta_yaml: Path = None) -> Factura:
    ...
    # v3.15/v3.17 - CARLOS NAVAS / Quesería Carlos Navas (incluye alias QUESERIA NAVAS)
