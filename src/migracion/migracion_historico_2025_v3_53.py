...
def extraer_texto_pdf(ruta: Path) -> str:
    """
    Extrae texto de un PDF usando sistema dual:
    1. Intenta con pdfplumber (mejor para tablas)
    2. Si no hay texto, intenta con pypdf
    3. Si sigue sin texto, usa Tesseract OCR (para PDFs escaneados)
    ...
    # PASO 1: Intentar con pdfplumber (mejor para tablas)
    if PDFPLUMBER_DISPONIBLE:
        try:
            ...
            pass  # Silencioso, intentamos con pypdf
    
    # PASO 2: Fallback a pypdf
    try:
        ...
        pass  # Silencioso, intentamos con OCR
    
    # PASO 3: OCR para PDFs escaneados (imágenes)
    if OCR_DISPONIBLE and not texto.strip():
        try:
            # Convertir PDF a imágenes
            images = convert_from_path(str(ruta), dpi=300)
            
            for img in images:
                # Intentar primero con español, si falla con inglés
                try:
...
def extraer_lineas_ceres(texto: str) -> List[Dict]:
    ...
    v3.36 - Soporte para CLA (caja retornable)
    v3.37 - Incluye ENVASE (pueden ser negativos por devoluciones)
    v3.39 - Productos sin descuento (SOUSAS, PLASTICO)
    
    Formatos detectados por pypdf:
    - Con descuento: 001124 MH CLASICA BARRIL 30L 5 21108,8 50 272,00
      (CODIGO DESC UDS IVA+PRECIO DTO IMPORTE)
    - Sin descuento: 001229 SOUSAS GAS MEDIO RT 20 UDS 1 109,49 9,49
      (CODIGO DESC UDS IVA+PRECIO IMPORTE)
    - Sin descuento: CE990020 PLASTICO 1/3 1 211 1,00
      (CODIGO DESC UDS IVA+PRECIO IMPORTE)
    - Envases: CE990002 ENVASE 30 lit. -5 2130 -150,00
    """
    lineas = []
    codigos_capturados = set()  # Para evitar duplicados
    
    # Patron 1: Productos CON descuento (mayoría)
    # Ejemplo: 001124 MH CLASICA BARRIL 30L 5 21108,8 50 272,00
    patron_con_descuento = re.compile(
        r'^([A-Z0-9]{5,8})\s+'  # Codigo
        r'([A-Z][A-Z0-9\s\'/\-\.]+?)\s+'  # Descripcion
        r'(-?\d+)\s+'  # Unidades
        r'(\d{1,2})'  # IVA (pegado al precio)
        r'(\d+[,\.]\d+)\s+'  # Precio
        r'(\d+[,\.]?\d*)\s+'  # Descuento
        r'(-?\d+[,\.]\d+)',  # Importe
        re.MULTILINE
    ...
    # Patron 2: Productos SIN descuento (SOUSAS, PLASTICO, etc.)
    # Ejemplo: 001229 SOUSAS GAS MEDIO RT 20 UDS 1 109,49 9,49
    # Ejemplo: CE990020 PLASTICO 1/3 1 211 1,00
    patron_sin_descuento = re.compile(
        r'^([A-Z0-9]{6,8})\s+'  # Codigo (6-8 chars: 001229 o CE990020)
        r'([A-Z][A-Z0-9\s\'/\-\.]+?)\s+'  # Descripcion
        r'(\d+)\s+'  # Unidades
        r'(\d{1,2})'  # IVA (pegado al precio)
        r'(\d+[,\.]?\d*)\s+'  # Precio (puede ser entero o decimal)
        r'(\d+[,\.]\d{2})$',  # Importe (siempre 2 decimales, fin de linea)
        re.MULTILINE
    )
    
    # Patron 3: Envases
    # Ejemplo: CE990002 ENVASE 30 lit. -5 2130 -150,00
    patron_envases = re.compile(
        r'^(CE99\d{4})\s+'  # Codigo CE99xxxx
        r'(ENVASE\s+\d+\s*lit\.?)\s+'  # Descripcion ENVASE XX lit.
    ...
    # Extraer productos SIN descuento (SOUSAS, PLASTICO, etc.)
    for match in patron_sin_descuento.finditer(texto):
        ...
        # Evitar duplicados con patron_con_descuento
        key = (codigo, float(importe.replace(',', '.')))
        if key in codigos_capturados:
            continue
        
        # Solo capturar si tiene importe > 0 (ignorar regalos)
    ...
    # v3.36 - Capturar CLA (caja retornable)
...
def extraer_lineas_molienda_verde(texto: str) -> List[Dict]:
    ...
    v3.52 - Compatible con pypdf y pdfplumber
    
    Formato: Código(3 dígitos) | Descripción | Cant | Precio | Dto | %IVA | Importe(BASE)
    Las descripciones pueden ocupar 2 líneas.
    ...
    # Limpiar espacios extra de pypdf (M IEL -> MIEL)
    texto_limpio = re.sub(r'(\w)\s(\w)\s(\w)\s(\w)', r'\1\2\3\4', texto)
    texto_limpio = re.sub(r'(\w)\s(\w)\s(\w)', r'\1\2\3', texto_limpio)
    texto_limpio = re.sub(r'(\w)\s(\w)', r'\1\2', texto_limpio)
    
    # Preprocesar: unir líneas con descripción partida
    lineas_texto = texto_limpio.split('\n')
    lineas_unidas = []
    i = 0
    while i < len(lineas_texto):
        linea = lineas_texto[i].strip()
        if re.match(r'^\d{3}\s+', linea):
            # Verificar si está completa (termina con patrón numérico)
    ...
    for match in patron_linea.finditer(texto_procesado):
        codigo, desc, cant, precio_base, dto, iva, importe_base = match.groups()
        iva_pct = int(iva)
        base = float(importe_base.replace(',', '.'))
    ...
    # Patrón para PORTES
    patron_portes = re.compile(r'PORTES?\s+(\d+)\s+(\d+[.,]\d{2})\s+(\d+)\s+(\d+)\s+(\d+[.,]\d{2})', re.IGNORECASE)
    m = patron_portes.search(texto_procesado)
    if m:
    ...
    # FALLBACK: Si no se extrajeron líneas, usar bases declaradas
    if not lineas:
        # Buscar bases en el desglose fiscal
        # Formato pypdf: "243,00 Euros207,27\n12,40\n10\n21..."
        
        # Patrón específico: Euros seguido de base10 y base21
        bases_match = re.search(r'Euros(\d+[.,]\d{2})\s*\n?\s*(\d+[.,]\d{2})\s*\n?\s*10\s*\n?\s*21', texto)
        if bases_match:
            base_10 = float(bases_match.group(1).replace(',', '.'))
            base_21 = float(bases_match.group(2).replace(',', '.'))
            ...
            if base_21 > 0 and base_21 < 50:  # Portes < 50€
                lineas.append({
                    'codigo': 'PORTES',
                    'articulo': 'PORTES',
                    'iva': 21,
...
def extraer_lineas_felisa(texto: str) -> List[Dict]:
    ...
    v3.40 - Corregido: el codigo esta PEGADO al importe
    Formato real: DESCRIPCION PRECIO(4dec) IMPORTE+CODIGO(pegados) CANTIDAD Unidades
    
    Ejemplo texto extraido:
        SARDINA AHUMADA 120GR OVAL 3,4000 34,00SAH120 10 Unidades
        ANCHOA AHUMADA 120GR OVAL 3,4000 20,40AAH120 6 Unidades
        SARDINA AHUMADA 500GR FELISA 11,4900 34,47FSAH500 3 Unidades
    
    IMPORTANTE: TRANSPORTE NO se extrae como linea - se prorratea automaticamente.
    """
    lineas = []
    
    # Patron: DESCRIPCION PRECIO(4dec) IMPORTE+CODIGO(pegados) CANTIDAD Unidades/Kilos
    # El importe tiene 2 decimales y el codigo empieza con letra
    patron = re.compile(
        r'^([A-Z][A-Z0-9\s]+?)\s+'   # Descripcion (empieza con letra)
        r'(\d+,\d{4})\s+'             # Precio (4 decimales)
        r'(\d+,\d{2})'                # Importe (2 decimales)
        r'([A-Z][A-Z0-9]+)\s+'        # Codigo PEGADO al importe
        r'([\d,]+)\s+'                # Cantidad
        r'(?:Unidades|Kilos)',
        re.MULTILINE
    )
    ...
    # El dispatcher ya tiene logica para prorratear TRANSPORTE/PORTES entre productos
    if 'TRANSPORTE' in texto:
        # Formato: TRANSPORTE seguido del total productos, luego salto linea, luego valor transporte
        # Ejemplo: "TRANSPORTE\n88,87\n8,30" donde 8,30 es el transporte
        ...
        if match_transp:
            ...
            # Alternativa: buscar en desglose fiscal "8,30 21 %" 
            match_transp2 = re.search(r'(\d+,\d{2})\s+21\s*%', texto)
            if match_transp2:
                valor = float(match_transp2.group(1).replace(',', '.'))
                if valor < 20:  # El transporte suele ser menos de 20 EUR
                    lineas.append({
                        'codigo': 'TRANSP',
                        'articulo': 'TRANSPORTE',
                        'iva': 21,
