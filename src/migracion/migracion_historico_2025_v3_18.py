...
# =============================================================================
# v3.18 - CATEGORÍAS POR DEFECTO PARA PROVEEDORES CONOCIDOS
# =============================================================================
...
def obtener_categoria_default(proveedor: str, articulo: str) -> str:
    """
    v3.18: Obtiene categoría por defecto basada en proveedor o palabras clave.
...
def extraer_fecha(texto: str, yaml_config: Optional[Dict] = None, proveedor: str = '') -> Optional[str]:
    ...
    # v3.18 - QUESOS FELIX / ARMANDO SANZ
    # PyPDF2 extrae: "24 000720 03/12/2024" (año + número + fecha)
    ...
    if 'FELIX' in proveedor_upper or 'ARMANDO SANZ' in proveedor_upper or 'B47440136' in texto or 'B 47.440.136' in texto:
        # Patrón: NN NNNNNN DD/MM/YYYY (5-6 dígitos en el número)
        patron = re.search(r'\d{2}\s+\d{5,6}\s+(\d{2})/(\d{2})/(\d{4})', texto)
        if patron:
            d, m, y = patron.groups()
            return f"{d}-{m}-{y[-2:]}"
...
def extraer_lineas_quesos_felix(texto: str) -> List[Dict]:
    """Extrae líneas de facturas QUESOS FELIX / Armando Sanz - v3.18.
    ...
    # v3.18: Patrón corregido - LOTE al final
...
def buscar_categoria(proveedor: str, articulo: str, indice: Dict, umbral: float = 0.8) -> Tuple[str, int]:
    ...
    if not proveedor_encontrado:
        # v3.18: Fallback - usar categoría por defecto
    ...
    # v3.18: Fallback - usar categoría por defecto basada en proveedor/artículo
    categoria_default = obtener_categoria_default(proveedor, articulo)
