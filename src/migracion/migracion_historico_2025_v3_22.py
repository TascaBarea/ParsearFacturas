#!/usr/bin/env python3
"""
MIGRACIÓN HISTÓRICO 2025 - v3.22
================================
...
def extraer_total(texto: str) -> Optional[float]:
    """Extrae el total de la factura - v3.21 mejorado con más patrones."""
    
    # Patrones ordenados de más específico a más genérico
    patrones = [
        # Patrón 1: TOTAL FACTURA / TOTAL IMPORTE
        r'(?:TOTAL\s*FACTURA|TOTAL\s*IMPORTE|Total\s*Factura)[:\s]*(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€?',
        # Patrón 2: TOTAL A PAGAR (común en tickets)
        r'TOTAL\s*A\s*PAGAR[:\s]*(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€?',
        # Patrón 3: TOTAL solo con € después
        r'TOTAL[:\s]+(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€',
        # Patrón 4: Importe Total
        r'Importe\s*Total[:\s]*(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€?',
        # Patrón 5: TOTAL COMPRA (BM, supermercados)
        r'TOTAL\s*COMPRA[^0-9]*(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})',
        # Patrón 6: € al final de línea con TOTAL
        r'TOTAL[^€\n]*?(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€',
        # Patrón 7: MARTIN ABENZA - línea con múltiples IMPORTE, tomar el último antes de €
        r'^\d{1,3}[.,]\d{2}\s+IMPORTE.*?(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€\s*$',
        # Patrón 8: Último importe con € en la línea de totales
        r'TOTAL\s+BRUTO.*?(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€\s*$',
    ]
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Migración histórico 2025 v3.22')
    ...
    print("="*60)
    print("MIGRACIÓN HISTÓRICO 2025 - v3.22")
    print("="*60)
