...
def extraer_lineas_emjamesa(texto: str) -> List[Dict]:
    ...
    if len(matches_con_iva) >= len(matches_sin_iva):
        for match in matches_con_iva:
            ...
            if codigo == '01':  # Portes se procesan aparte
                continue
...
def extraer_lineas_rosquilleria(texto: str) -> List[Dict]:
    """
    Extractor para LA ROSQUILLERIA S.L.U. (OCR)
    
    Formato OCR:
        RN-1.15 ROSQUILLA 3825 6 15 90 1,02€ 4% 91,80
                ORIGINAL
        GSE 2   GASTOS DE ENVIO 1 1 20,00€ 0% 20,00
    
    Los GASTOS DE ENVIO se mantienen como línea (0% IVA)
    """
    lineas = []
    
    # Patrón principal: REF DESCRIPCION ... PRECIO€ IVA% IMPORTE
    patron = re.compile(
        r'([A-Z]{2,4}[-\s]?\d{0,4}\.?\d*)\s+'    # Referencia (RN-1.15, GSE 2)
        r'([A-Z][A-Z\s]+?)\s+'                     # Descripción
        r'.*?'                                      # Campos intermedios
        r'(\d{1,3}[,\.]\d{2})\s*€?\s+'             # Precio unitario
        r'(\d{1,2})%\s*'                           # IVA
    ...
    for match in patron.finditer(texto):
        ...
        try:
            ...
            lineas.append({
                ...
                'articulo': desc.strip(),
                'iva': iva_int,
                'base': base  # El importe ya es la base
            })
        except ValueError:
            continue
    
    # Si el patrón anterior no funcionó, intentar uno más simple
    if not lineas:
        patron_simple = re.compile(
            r'(ROSQUILLA[A-Z\s]*)\s+.*?(\d{1,2})%\s*(\d{1,3}[,\.]\d{2})',
            re.IGNORECASE
        )
        
        for match in patron_simple.finditer(texto):
            desc, iva, importe = match.groups()
...
def procesar_factura(ruta: Path, indice: Dict, carpeta_yaml: Path = None) -> Factura:
    ...
    # v3.53 - LA ROSQUILLERIA (OCR)
        ...
        # Detectar gastos de envío y repartir
        envio_match = re.search(r'GASTOS\s+DE\s+ENVIO.*?(\d+[,\.]\d{2})', texto, re.IGNORECASE)
        if envio_match and lineas_raw and factura.total:
            base_envio = float(envio_match.group(1).replace(',', '.'))
            if base_envio > 0:
                # Los gastos de envío van al 0% IVA, repartir entre artículos
                lineas_raw = repartir_portes(lineas_raw, factura.total, base_envio, iva_portes=0)
