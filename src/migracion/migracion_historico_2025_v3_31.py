...
def extraer_total(texto: str) -> Optional[float]:
    """Extrae el total de la factura - v3.30b con SEGURMA multiline."""
    ...
    patrones = [
        ...
        # v3.30b - Patrón 10: Total XX,XX € (SEGURMA - con posibles saltos de línea)
        # Usa (?<!Sub) para evitar "Subtotal" y \s* para manejar saltos de línea
        r'(?<!Sub)Total[\s\n]*(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})[\s\n]*€',
    ]
