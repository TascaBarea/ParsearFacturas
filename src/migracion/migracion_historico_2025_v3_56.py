...
def extraer_lineas_jimeluz(texto: str) -> List[Dict]:
    """
    Extractor para JIMELUZ EMPRENDEDORES S.L. (OCR de tickets escaneados)
    
    Formato (ticket de supermercado):
        CANT DESCRIPCION    % IVA    IMPORTE
        1 LIMON CDC         4,00     1,52
        2 HIELO CUBITOS 2 KG 10,00   2,50
    
    El texto viene de OCR, puede tener errores de lectura.
    """
    lineas = []
    
    # Buscar zona de artículos (entre "DESCRIPCION" y "TOTAL COMPRA")
    inicio = re.search(r'DESCRIPCION\s+%\s*IVA\s+IMPORTE', texto, re.IGNORECASE)
    ...
    for linea_texto in zona_articulos.split('\n'):
        linea_texto = linea_texto.strip()
        if not linea_texto:
            continue
        
        # Patrón flexible para OCR:
        # CANTIDAD DESCRIPCION IVA(con ruido) IMPORTE
        # IVA puede ser: 4,00 o 4.00 o 4.90 (ruido) o "4 00"
        # Importe puede ser: 1,52 o 1.52 o "1 58" (espacio por OCR)
        match = re.match(
            r'^(\d+)\s+'                           # Cantidad
            r'(.+?)\s+'                            # Descripción  
            r'(\d{1,2})[,.\s]?\d*\s+'              # IVA (flexible, ignora decimales)
            r'(\d+[\s,.]\d{2})',                   # Importe
            linea_texto
        )
        
        if match:
            cantidad, descripcion, iva, importe_raw = match.groups()
            
            # Limpiar importe: espacio -> nada, coma -> punto
