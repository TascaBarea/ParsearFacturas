...
v3.38 - CERES: SOUSAS y PLASTICO
      - Nuevo patron para productos con IVA separado (SOUSAS GAS al 10%)
      - Nuevo patron para CE990020 PLASTICO (sin descuento)
      - Corrige descuadres -10.44 EUR y -1.21 EUR

v3.37 - CERES ENVASES (devoluciones)
      - Extractor CERES: incluye líneas ENVASE (pueden ser negativas)
      - Corrige descuadres por devolución de barriles (-150€, etc.)
...
def extraer_lineas_ceres(texto: str) -> List[Dict]:
    ...
    v3.38 - SOUSAS (IVA separado) y PLASTICO (CE990020)
    
    Formatos detectados:
    - Productos normales: CODIGO DESC UDS IVA+PRECIO DTO IMPORTE (ej: 001124 MH CLASICA 4 21108,8 50 217,60)
    - Productos IVA separado: CODIGO DESC UDS PRECIO IVA IMPORTE (ej: 001229 SOUSAS GAS 1 9,49 10 9,49)
    - Envases: CE990002 ENVASE 30 lit. -5 2130 -150,00
    - Plastico: CE990020 PLASTICO 1/3 1 1 21 1,00
    """
    lineas = []
    
    # Patron 1: Productos normales (IVA pegado al precio)
    # Ejemplo: 001124 MH CLASICA BARRIL 30L 4 21108,8 50 217,60
    patron_productos = re.compile(
        r'^([A-Z0-9]{5,6})\s+'  # Codigo (001124 o CE1384)
        r'([A-Z][A-Z0-9\s\'/\-\.]+?)\s+'  # Descripcion
        r'(-?\d+)\s+'  # Unidades (puede ser negativo)
        r'(\d{1,2})'  # IVA (21, 10, 4) - PEGADO al precio
    ...
    # Patron 2: Envases (IVA pegado al precio, SIN columna descuento)
    # Texto extraido: CE990002 ENVASE 30 lit. -5 2130 -150,00
    patron_envases = re.compile(
        r'^(CE\d{5,6})\s+'  # Codigo CE (CE990001, CE990002)
        r'(ENVASE\s+\d+\s*lit\.?)\s+'  # Descripcion ENVASE XX lit.
        r'(-?\d+)\s+'  # Unidades (puede ser negativo)
        r'(\d{1,2})'  # IVA (pegado al precio)
        r'(\d+)\s+'  # Precio (sin decimales: 30, 50)
        r'(-?\d+[,\.]\d+)',  # Importe
        re.MULTILINE
    )
    
    # v3.38 - Patron 3: Productos con IVA SEPARADO del precio (SOUSAS, etc.)
    # Texto extraido: 001229 SOUSAS GAS MEDIO RT 20 UDS 1 9,49 10 9,49
    # Formato: CODIGO DESC UDS PRECIO IVA IMPORTE (sin descuento, IVA separado)
    patron_iva_separado = re.compile(
        r'^(\d{6})\s+'  # Codigo numerico 6 digitos (001229)
        r'([A-Z][A-Z0-9\s\'/\-\.]+?)\s+'  # Descripcion
        r'(\d+)\s+'  # Unidades
        r'(\d+[,\.]\d{2})\s+'  # Precio con decimales
        r'(\d{1,2})\s+'  # IVA SEPARADO (10, 21, 4)
        r'(\d+[,\.]\d{2})',  # Importe
        re.MULTILINE
    )
    
    # v3.38 - Patron 4: PLASTICO y otros CE990xxx (sin descuento)
    # Texto extraido: CE990020 PLASTICO 1/3 1 1 21 1,00
    # Formato: CODIGO DESC UDS PRECIO IVA IMPORTE
    patron_plastico = re.compile(
        r'^(CE990\d{3})\s+'  # Codigo CE990xxx
        r'(PLASTICO[A-Z0-9\s/\-]*?)\s+'  # Descripcion PLASTICO...
        r'(\d+)\s+'  # Unidades
        r'(\d+)\s+'  # Precio (entero)
        r'(\d{1,2})\s+'  # IVA
        r'(\d+[,\.]\d{2})',  # Importe
    ...
    # v3.38 - Extraer productos con IVA separado (SOUSAS, etc.)
    for match in patron_iva_separado.finditer(texto):
        codigo, descripcion, uds, precio, iva, importe = match.groups()
        descripcion_limpia = descripcion.strip()
        # Evitar duplicados con patron_productos
        importe_val = float(importe.replace(',', '.'))
        if importe_val > 0:  # Solo si tiene importe positivo
            # Verificar que no sea una linea de regalo (importe 0)
            lineas.append({
                'codigo': codigo,
                'articulo': descripcion_limpia,
                'iva': int(iva),
...
def extraer_lineas_felisa(texto: str) -> List[Dict]:
    ...
    v3.32 - Corregido patrón TRANSPORTE: en el PDF "TRANSPORTE88,87\n8,30" 
    el valor real (8,30) está en la línea siguiente.
    """
    lineas = []
    ...
    # v3.32 - Buscar TRANSPORTE (IVA 21%)
    # El PDF tiene formato "TRANSPORTE88,87\n8,30" donde 8,30 es el valor real
    if 'TRANSPORTE' in texto:
        # Primero intentar patrón específico: TRANSPORTE + número + salto + número real
        match_transp = re.search(r'TRANSPORTE[\d,]+\n([\d,]+)', texto)
        if match_transp:
            lineas.append({
                'codigo': 'TRANSP',
                'articulo': 'TRANSPORTE',
            ...
            # Fallback: patrón antiguo por si cambia el formato
            match_transp2 = re.search(r'TRANSPORTE\s+([\d,]+)', texto)
            if match_transp2:
                lineas.append({
                    'codigo': 'TRANSP',
                    'articulo': 'TRANSPORTE',
                    'iva': 21,
                    'base': float(match_transp2.group(1).replace(',', '.'))
                })
