#!/usr/bin/env python3
"""
MIGRACIÓN HISTÓRICO 2025 - v3.15
================================
Extrae líneas detalladas de facturas, categoriza artículos,
prorratrea gastos y descuentos, y actualiza maestros.

v3.15 - Mejoras principales:
      - SOM ENERGIA mejorado:
        - Extrae FECHA correctamente (Fecha de la factura: DD/MM/YYYY)
        - Extrae REF correctamente (N.º de factura: FE...)
        - Distingue "Electricidad Tasca" vs "Electricidad Comestibles"
          según cuenta bancaria (4495=Tasca, 2404=Comestibles)
        - Categoría fija: ELECTRICIDAD
      - Añadido extractor QUESOS FELIX / ARMANDO SANZ
        CIF: B47440136, IBAN: ES40 2103 4346 3300 3306 5431
        IVA: 4%, categoría fija: QUESOS
      - Actualizado IBAN ZUBELZU: ES61 3035 0141 8214 1001 9635
      - Actualizado IBAN MARTIN ABENZA: ES37 0049 6193 4128 9534 3887
      - Corregido patrón REF de PRODUCTOS ADELL (A/XXX al inicio)
...
PROVEEDORES_CONOCIDOS = {
    ...
    # v3.12/v3.15 - GRUPO TERRITORIO CAMPERO (patatas fritas) - IBAN añadido v3.15
    ...
    # v3.12/v3.15 - ZUBELZU PIPARRAK (guindillas y mousse) - IBAN completo v3.15
    ...
    # v3.13/v3.15 - MARTIN ABENZA / El Modesto - IBAN añadido v3.15
...
def extraer_fecha(texto: str, yaml_config: Optional[Dict] = None, proveedor: str = '') -> Optional[str]:
    ...
    # v3.12/v3.15 - LA BARRA DULCE: Fecha DD.MM.YYYY (después del CIF)
    if 'BARRA DULCE' in proveedor_upper or 'B19981141' in texto:
        # v3.15: El texto extraído tiene formato "CIF: B19981141 30.11.2025"
    ...
    # v3.15 - SOM ENERGIA: "Fecha de la factura: DD/MM/YYYY"
    if 'SOM ENERGIA' in proveedor_upper or 'SOM_ENERGIA' in proveedor_upper or 'F55091367' in texto:
        patron = re.search(r'Fecha de la factura:\s*(\d{2})/(\d{2})/(\d{4})', texto)
        if patron:
            d, m, y = patron.groups()
            return f"{d}-{m}-{y[-2:]}"
    
    # v3.15 - QUESOS FELIX: "25 00026DD/MM/YYYY" (número factura pegado a fecha)
    if 'QUESOS' in proveedor_upper and 'FELIX' in proveedor_upper or 'ARMANDO SANZ' in proveedor_upper or 'B47440136' in texto or 'B 47.440.136' in texto:
        # Formato: "25 00026DD/MM/YYYY" - número de factura pegado a fecha
        patron = re.search(r'\d{2}\s+\d{5}(\d{2})/(\d{2})/(\d{4})', texto)
        if patron:
            d, m, y = patron.groups()
            return f"{d}-{m}-{y[-2:]}"
...
def extraer_ref(texto: str, yaml_config: Optional[Dict] = None, proveedor: str = '') -> Optional[str]:
    ...
    # v3.13/v3.15 - PRODUCTOS ADELL: "A/328" al principio del documento
    if 'ADELL' in proveedor_upper or 'CROQUELLANAS' in proveedor_upper or 'B12711636' in texto:
        # El número de factura A/XXX aparece al principio del documento
        patron = re.search(r'^(A/\d+)', texto, re.MULTILINE)
        if patron:
            return patron.group(1)
        # Fallback: buscar después de "Nº Factura"
        patron2 = re.search(r'N[ºo°]\s*Factura\s*\n?\s*(A/\d+)', texto, re.IGNORECASE)
        if patron2:
    ...
    # v3.15 - SOM ENERGIA: "N.º de factura:FE2501314182"
    if 'SOM ENERGIA' in proveedor_upper or 'SOM_ENERGIA' in proveedor_upper or 'F55091367' in texto:
        patron = re.search(r'N\.º de factura:\s*(FE\d+)', texto)
        if patron:
            return patron.group(1)
    
    # v3.15 - QUESOS FELIX: "25 00026" después de "MADRID\nMADRID"
    if 'QUESOS' in proveedor_upper and 'FELIX' in proveedor_upper or 'ARMANDO SANZ' in proveedor_upper or 'B47440136' in texto or 'B 47.440.136' in texto:
        # Formato: "25 00026" (año 2 dígitos + espacio + número 5 dígitos)
        patron = re.search(r'MADRID\s*\n\s*(\d{2}\s+\d{5})', texto)
        if patron:
            return patron.group(1).replace(' ', '-')
...
def extraer_lineas_som_energia(texto: str) -> List[Dict]:
    ...
    v3.15 - Mejorado:
    - Distingue "Electricidad Tasca" vs "Electricidad Comestibles"
      según la cuenta bancaria o la dirección de suministro
    - TASCA: cuenta termina en 4495, dirección sin "COMESTIBLES"
    - COMESTIBLES: cuenta termina en 2404, dirección contiene "COMESTIBLES"
    
    Siempre genera UNA línea con:
    - Artículo: "Electricidad Tasca" o "Electricidad Comestibles"
    - IVA: 21%
    - Base: extraída de "(BASE IMPONIBLE)"
    - Categoría fija: ELECTRICIDAD
    ...
    # Determinar si es TASCA o COMESTIBLES
    es_comestibles = False
    
    # Método 1: Buscar en dirección de suministro
    if 'COMESTIBLES' in texto.upper():
        es_comestibles = True
    
    # Método 2: Buscar últimos dígitos de cuenta bancaria
    # TASCA termina en 4495, COMESTIBLES termina en 2404
    cuenta_match = re.search(r'N\.º cuenta bancaria:\s*\*+\s*(\d+)', texto)
    if cuenta_match:
        ultimos_digitos = cuenta_match.group(1)
        if ultimos_digitos == '2404':
            es_comestibles = True
        elif ultimos_digitos == '4495':
            es_comestibles = False
    
    # Determinar artículo
    articulo = 'Electricidad Comestibles' if es_comestibles else 'Electricidad Tasca'
    
    # Extraer IVA (normalmente 21%)
    iva = 21
    patron_iva = re.search(r'IVA\s*(\d+)%\s*([\d,]+)\s*€?', texto)
    if patron_iva:
        iva = int(patron_iva.group(1))
    
    # Buscar base imponible - patrón principal: "173,98 € (BASE IMPONIBLE)"
    base = None
    patron_base = re.search(r'([\d.,]+)\s*€?\s*\(BASE\s*IMPONIBLE\)', texto, re.IGNORECASE)
    if patron_base:
        try:
            base_str = patron_base.group(1).replace('.', '').replace(',', '.')
            base = float(base_str)
        except:
            pass
    
    # Fallback: calcular TOTAL - IVA
    if base is None:
        patron_total = re.search(r'TOTAL\s*(?:IMPORTE\s*)?FACTURA[:\s]*([\d.,]+)\s*€?', texto, re.IGNORECASE)
        patron_iva_importe = re.search(r'IVA\s*\d+%\s*([\d,]+)\s*€?', texto)
        if patron_total and patron_iva_importe:
            try:
                total_str = patron_total.group(1).replace('.', '').replace(',', '.')
                iva_str = patron_iva_importe.group(1).replace(',', '.')
                total = float(total_str)
                importe_iva = float(iva_str)
    ...
    if base and base > 0:
        lineas.append({
            ...
            'iva': iva,
            'base': round(base, 2),
            'categoria': 'ELECTRICIDAD'  # Categoría fija
        })
...
def extraer_lineas_quesos_felix(texto: str) -> List[Dict]:
    """Extrae líneas de facturas QUESOS FELIX / ARMANDO SANZ - v3.15.
    
    CIF: B47440136
    IBAN: ES40 2103 4346 3300 3306 5431
    Productos: Quesos de oveja (Félix, Patamulo, IGP)
    IVA: 4% (quesos)
    Categoría fija: QUESOS
    
    Formato OCR peculiar - las líneas vienen como:
    DESCRIPCION UDS CANTIDAD PRECIO IMPORTE€LOTE
    
    Ejemplo:
    "QUESO IGP FELIX. SEMICURADO GRANDE 1 3,100 14,00 43,40 €4195"
    "QUESO FELIX. Especial PATAMULO GRANDE 3 4,500 15,00 67,50 €4201"
    """
    lineas = []
    
    # Patrón: DESCRIPCION (empieza con QUESO) UDS CANTIDAD PRECIO IMPORTE€LOTE
    patron = re.compile(
        r'(QUESO[^\d]+?)\s+'      # Descripción (empieza con QUESO, hasta dígitos)
        r'(\d+)\s+'               # UDS (unidades enteras)
        r'(\d+[,.]\d{3})\s+'      # CANTIDAD (con 3 decimales: 3,100)
        r'(\d+[,.]\d{2})\s+'      # PRECIO (con 2 decimales: 14,00)
        r'(\d+[,.]\d{2})\s*€?'    # IMPORTE (con 2 decimales: 43,40)
        r'[\d-]*',                # LOTE opcional (4195 o -)
        re.MULTILINE
    )
    
    for match in patron.finditer(texto):
        descripcion, uds, cantidad, precio, importe = match.groups()
        
        # Limpiar descripción
        desc_limpia = descripcion.strip()
        # Quitar "." al final si existe
        if desc_limpia.endswith('.'):
            desc_limpia = desc_limpia[:-1].strip()
...
def procesar_factura(ruta: Path, indice: Dict, carpeta_yaml: Path = None) -> Factura:
    ...
    # v3.15 - QUESOS FELIX / ARMANDO SANZ
    elif 'QUESOS' in proveedor_upper and 'FELIX' in proveedor_upper or 'ARMANDO SANZ' in proveedor_upper or 'B47440136' in texto or 'B 47.440.136' in texto:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Migración histórico 2025 v3.15')
    ...
    print("="*60)
    print("MIGRACIÓN HISTÓRICO 2025 - v3.15")
    print("="*60)
