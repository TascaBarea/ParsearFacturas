...
v3.48 - FIX CERES CE99 SIN DESCUENTO + FELISA GOURMET
      - CERES: Añadido patrón CE99 sin descuento (PLASTICO 1/3, etc)
      - FELISA GOURMET: Reescrito extractor para formato pdfplumber
      - Soporta múltiples IVAs (10% productos, 21% transporte)
...
def extraer_lineas_ceres(texto: str) -> List[Dict]:
    ...
    # Patrón 4b: CE99 SIN descuento (PLASTICO, etc) - pdfplumber
    # CE990020 PLASTICO 1/3 1 1 21 1,00
    patron_pdfplumber_ce99_sin_dto = re.compile(
        r'^(CE99\d{4})\s+'               # Codigo CE99xxxx
        r'([A-Z][A-Z0-9\s/]+?)\s+'       # Descripcion
        r'(-?\d+)\s+'                    # Unidades
        r'(\d+)\s+'                      # Precio
        r'(\d{1,2})\s+'                  # IVA
        r'(-?\d+[,\.]\d{2})$',           # Importe (fin de línea)
        re.MULTILINE
    )
    ...
    # 4b. CE99 sin descuento (PLASTICO, etc) - pdfplumber
    for m in patron_pdfplumber_ce99_sin_dto.finditer(texto):
        codigo, desc, uds, precio, iva, importe = m.groups()
        key = (codigo, float(importe.replace(',', '.')))
        if key not in codigos_capturados:
            codigos_capturados.add(key)
...
def extraer_lineas_felisa(texto: str) -> List[Dict]:
    ...
    v3.48 - Reescrito para formato pdfplumber
    
    Formato:
    - Productos: SAH120 SARDINA AHUMADA 120GR OVAL 10 Unidades 3,4000 34,00
    - Transporte: TRANSPORTE 8,30 (21% IVA)
    - Productos llevan 10% IVA, transporte 21%
    """
    lineas = []
    
    # Patrón productos: CODIGO DESC CANTIDAD Unidades/Kilos PRECIO TOTAL
    patron_producto = re.compile(
        r'^([A-Z]{2,}\d*)\s+'           # Codigo (SAH120, AAH120, FSAH500, TN, BV120)
        r'([A-Z][A-Z0-9\s]+?)\s+'       # Descripcion
        r'([\d,\.]+)\s+'                # Cantidad
    ...
    for m in patron_producto.finditer(texto):
        codigo, desc, cantidad, unidad, precio, total = m.groups()
        lineas.append({
            'codigo': codigo,
            'articulo': desc.strip(),
            'iva': 10,  # Productos alimentarios al 10%
            'base': float(total.replace(',', '.'))
        })
    
    # Transporte (21% IVA)
    transporte_match = re.search(r'TRANSPORTE\s+([\d,\.]+)', texto)
    if transporte_match:
        lineas.append({
            'codigo': 'TRANSPORTE',
            'articulo': 'Portes',
            'iva': 21,
            'base': float(transporte_match.group(1).replace(',', '.'))
        })
...
def procesar_factura(ruta: Path, indice: Dict, carpeta_yaml: Path = None) -> Factura:
    ...
    # v3.27 - LA MOLIENDA VERDE (antes del genérico)
        ...
        # v3.48 - Extraer total específico para FELISA
        if factura.total is None:
            total_match = re.search(r'Total Factura\s+([\d,\.]+)', texto)
            if total_match:
                factura.total = float(total_match.group(1).replace(',', '.'))
