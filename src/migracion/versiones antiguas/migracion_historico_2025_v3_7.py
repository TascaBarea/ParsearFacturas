...
def extraer_lineas_fabeiro(texto: str) -> List[Dict]:
    """Extrae líneas de facturas FABEIRO.
    
    CIF: B79992079
    
    Formato tabla:
    Artículo | Concepto                              | I.V.A. | Cantidad | P.Unidad | Importe
    CA0005   | ANCHOA OLIVA GRAN SELECCION 60 LOMOS  | 10,00% | 12,0000  | 24,0000  | 288,00
    
    IVA: Explícito en factura (4% quesos, 10% charcutería)
    Particularidad: OCR puede intercambiar posición de columnas
    """
    lineas = []
    
    # Patrón principal: CODIGO CONCEPTO IVA% CANTIDAD PRECIO IMPORTE
    patron = re.compile(
        r'^([A-Z]{2}\d{4})\s+'  # Código (CA0005, etc.)
        r'(.+?)\s+'  # Concepto
        r'(\d{1,2})[,.]00\s*%?\s+'  # IVA (10,00% o 4,00%)
        r'[\d,]+\s+'  # Cantidad (ignorar)
    ...
    # Patrón alternativo (OCR con columnas desordenadas)
    if not lineas:
        patron_alt = re.compile(
            r'^([A-Z]{2}\d{4})\s+'  # Código
            r'(.+?)\s+'  # Concepto
            r'(\d+[.,]\d{2})\s+'  # Importe (puede venir antes del IVA)
            r'(\d{1,2})[,.]00',  # IVA
            re.MULTILINE
        )
        
        for match in patron_alt.finditer(texto):
            codigo, concepto, importe, iva = match.groups()
            concepto_limpio = concepto.strip()
            
            if len(concepto_limpio) < 5:
                continue
...
def extraer_lineas_serrin(texto: str) -> List[Dict]:
    """Extrae líneas de facturas SERRÍN NO CHAN.
    
    CIF: B87214755
    IBAN: ES88 0049 6650 1329 1001 8834
    
    Formato tabla muy limpio:
    Código | Referencia/Artículo      | Cantidad | Precio Unitario | Tipo I.V.A | Importe sin I.V.A. | Importe con I.V.A.
    MARI   | Mariñeiras AOVE ECO      | 10       | 1,89            | 10,00%     | 18,90 €            | 20,79 €
    
    IVA: Explícito y variable (4% galletas ECO, 10% normales, 21% vermú)
    Validación: Sum "Importe sin I.V.A." = "Total Factura sin I.V.A."
    """
    lineas = []
    
    # Patrón: CODIGO | ARTICULO | CANT | PRECIO | IVA% | BASE | TOTAL
    patron = re.compile(
        r'^([A-Z]{3,6})\s+'  # Código corto (MARI, VERM, etc.)
        r'(.+?)\s+'  # Artículo
        r'(\d+)\s+'  # Cantidad
        r'[\d,]+\s+'  # Precio unitario (ignorar)
        r'(\d{1,2})[,.]00\s*%?\s+'  # IVA (4,00%, 10,00%, 21,00%)
        r'(\d+[.,]\d{2})\s*€?\s+'  # Base (Importe sin IVA)
        r'\d+[.,]\d{2}',  # Total con IVA (ignorar, usamos base)
        re.MULTILINE
    )
    
    for match in patron.finditer(texto):
        codigo, articulo, cant, iva, base = match.groups()
        articulo_limpio = articulo.strip()
        
        # Ignorar cabeceras
        if any(x in articulo_limpio.upper() for x in ['REFERENCIA', 'ARTÍCULO', 'DESCRIPCIÓN']):
            continue
    ...
    # Patrón alternativo si el primero no funciona
    if not lineas:
        # Buscar líneas con formato más flexible
        patron_alt = re.compile(
            r'([A-Z]{3,6})\s+'  # Código
        ...
        for match in patron_alt.finditer(texto):
            codigo, articulo, iva, base = match.groups()
