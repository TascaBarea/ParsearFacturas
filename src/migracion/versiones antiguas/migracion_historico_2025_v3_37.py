...
def extraer_lineas_ceres(texto: str) -> List[Dict]:
    ...
    v3.36 - Añadido soporte para CLA (caja retornable)
    v3.37 - Incluye ENVASE (pueden ser negativos por devoluciones)
    
    Formato productos: CODIGO DESCRIPCION UDS IVA+PRECIO DTO IMPORTE
    Ejemplo: 001124 MH CLASICA BARRIL 30L 4 21108,8 50 217,60
    
    Formato envases: CE990002 ENVASE 30 lit. -5 30 0 21 -150,00
    """
    lineas = []
    
    # Patrón 1: Productos normales (IVA pegado al precio)
    # Ejemplo: 001124 MH CLASICA BARRIL 30L 4 21108,8 50 217,60
    ...
    # Patrón 2: Envases (IVA pegado al precio, SIN columna descuento)
    # Texto extraído: CE990002 ENVASE 30 lit. -5 2130 -150,00
    # Formato: CODIGO DESCRIPCION UDS IVA+PRECIO IMPORTE
    patron_envases = re.compile(
        r'^(CE\d{5,6})\s+'  # Código CE (CE990001, CE990002)
        r'(ENVASE\s+\d+\s*lit\.?)\s+'  # Descripción ENVASE XX lit.
        r'(-?\d+)\s+'  # Unidades (puede ser negativo)
    ...
    cla_match = re.search(r'CLA:\s*(\d+)\s*€', texto)
    if cla_match:
