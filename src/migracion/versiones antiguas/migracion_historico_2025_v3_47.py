...
v3.47 - FIX CERES PRODUCTOS SIN DESCUENTO
      - Añadido patrón para productos 6 dígitos SIN descuento (ej: SOUSAS)
      - Ignora líneas con importe 0,00 (envases sin cargo)
      
v3.46 - FIX CERES PRODUCTOS CE1xxx (1T25)
      - Añadido patrón para códigos CE1xxx (ALH ESP, VERMUT, etc.)
      - Soporte descuentos con decimales (22,5%)
      
v3.45 - FIX CERES TOTAL NEGATIVOS
      - Corregido extractor de total para CERES (soporta valores negativos)
      - Patrón dual: pdfplumber (puntos + número) y pypdf (número separado)
      
v3.44 - ADAPTACIÓN PATRONES PARA PDFPLUMBER
      - LICORES MADRUEÑO: nuevo patrón para formato tabular pdfplumber
      - FRANCISCO GUERRA: nuevo patrón para columnas reordenadas
      - JAMONES BERNAL: nuevo patrón para formato pdfplumber
      - SERRIN NO CHAN: nuevo patrón para formato tabular completo
      - Patrones duales: soportan tanto pypdf como pdfplumber

v3.42 - CAMBIO EXTRACTOR PDF: pypdf → pdfplumber
      - pdfplumber preserva estructura de líneas (cantidad+descripción+precio juntos)
      - Mejora extracción en MARTIN ABENZA, DISBER, y facturas tabulares
      - Fallback a pypdf si pdfplumber falla
      - Benchmark: misma velocidad, mejor calidad de texto
...
def extraer_texto_pdf(ruta: Path) -> str:
    """
    Extrae texto de un PDF usando pdfplumber (preferido) con fallback a pypdf.
    
    pdfplumber preserva mejor la estructura de líneas en facturas tabulares:
    - pypdf:      "1 34,20 34,20 €" + "LOTES CONSERVAS" (separado)
    - pdfplumber: "1 LOTES CONSERVAS VARIADAS 34,20 34,20 €" (junto)
    """
    # Intento 1: pdfplumber (mejor para facturas)
    if USE_PDFPLUMBER:
        try:
            texto = ""
            ...
            print(f"    ⚠️ pdfplumber falló, usando pypdf: {e}")
    
    # Intento 2: pypdf/PyPDF2 (fallback)
    try:
...
def extraer_total(texto: str) -> Optional[float]:
    """Extrae el total de la factura - v3.44 con patrones FRANCISCO GUERRA y CERES."""
    """Extrae el total de la factura - v3.44 con patrón FRANCISCO GUERRA."""
    ...
    # v3.44: Añadido patrón FRANCISCO GUERRA (BASE TOTAL €)
    patrones = [
        # v3.44 - Patrón CERES - "Importe TOTAL ........ XX,XX"
        # Soporta valores negativos (devoluciones)
        r'Importe\s*TOTAL\s*\.+\s*(-?\d{1,3}(?:[.,]\d{3})*[.,]\d{1,2})',
        # v3.44 - Patrón FRANCISCO GUERRA - "BASE TOTAL €" en línea final
        # Formato: "602,13 662,34 €" donde 662,34 es el TOTAL
        # Busca dos números seguidos donde el segundo va antes de €
        r'^\d+[,\.]\d{2}\s+(\d{1,3}(?:[.,]\d{3})*[.,]\d{2})\s*€\s*$',
...
def extraer_lineas_licores_madrueño(texto: str) -> List[Dict]:
    ...
    1. pypdf (pegado): 1764 12XIC DA L FONS 43,203,60
       - Unidades PEGADAS a descripción
       - Importe+precio PEGADOS
    
    2. pdfplumber (tabular): 1764 XIC DAL FONS 12 3,60 43,20
       - Columnas separadas: CODIGO DESC UDS PRECIO IMPORTE
    """
    lineas = []
    
    # PATRÓN 1: pdfplumber (tabular limpio) - PRIORITARIO
    # Formato: CODIGO DESCRIPCION UDS PRECIO IMPORTE
    ...
    patron_pdfplumber = re.compile(
        ...
        r'(\d{1,3})\s+'                     # Unidades (separado)
        r'(\d{1,3}[,\.]\d{2})\s+'           # Precio unitario
        r'(\d{1,4}[,\.]\d{2})',             # Importe
        re.MULTILINE
    )
    
    for match in patron_pdfplumber.finditer(texto):
        codigo, descripcion, uds, precio, importe = match.groups()
        descripcion_limpia = ' '.join(descripcion.split()).strip()
        
        # Ignorar líneas de headers/totales
        desc_upper = descripcion_limpia.upper()
        if any(x in desc_upper for x in ['ALBAR', 'TOTAL', 'BRUTO', 'SUMA', 'SIGUE', 'BASE', 'IVA', 'FECHA']):
            continue
    ...
    # Si encontramos líneas con patrón pdfplumber, retornar
    if lineas:
        return lineas
    
    # PATRÓN 2: pypdf (formato pegado) - FALLBACK
    # Formato: CODIGO UDS+DESC IMPORTE,PRECIO (todo pegado)
    ...
    for match in patron_pypdf.finditer(texto):
...
def extraer_lineas_francisco_guerra(texto: str) -> List[Dict]:
    """Extrae líneas de facturas FRANCISCO GUERRA - v3.44 con soporte pdfplumber.
    
    Soporta DOS formatos con diferente orden de columnas:
    
    1. pypdf: CODIGO DESCRIPCION CANTIDAD TOTAL PRECIO_UD
       Ejemplo: 00192 CHUPADEO ZAMBUDIO      4         84,28     21,070
       El TOTAL viene antes del PRECIO (precio con 3 decimales)
    
    2. pdfplumber: CODIGO DESCRIPCION CANTIDAD PRECIO TOTAL
       Ejemplo: 00192 CHUPADEO ZAMBUDIO 2 21,070 42,14
       El PRECIO viene antes del TOTAL
    
    NOTA: El TOTAL de cada línea YA ES LA BASE (sin IVA).
    """
    lineas = []
    
    # PATRÓN 1: pdfplumber (CANTIDAD PRECIO TOTAL) - PRIORITARIO
    # Formato: CODIGO DESC CANTIDAD PRECIO(3dec) TOTAL(2dec)
    patron_pdfplumber = re.compile(
        r'^(\d{3,5})\s+'                    # Código (3-5 dígitos)
        r'(.+?)\s+'                          # Descripción
        r'(\d+)\s+'                          # Cantidad
        r'(\d+[,\.]\d{2,3})\s+'             # Precio (2-3 decimales)
        r'(\d+[,\.]\d{2})$',                # Total = BASE (2 decimales)
        re.MULTILINE
    )
    
    for match in patron_pdfplumber.finditer(texto):
        codigo, articulo, cantidad, precio, total = match.groups()
        ...
        # Si precio tiene 3 decimales, es formato pdfplumber correcto
    ...
    # PATRÓN 2: pypdf (CANTIDAD TOTAL PRECIO) - FALLBACK
    # Formato: CODIGO DESC CANTIDAD TOTAL(2dec) PRECIO(3dec)
    patron_pypdf = re.compile(
        r'^(\d{3,5})\s+'                    # Código (3-5 dígitos)
...
def extraer_lineas_ceres(texto: str) -> List[Dict]:
    ...
    v3.44 - Dual pdfplumber/pypdf con patrones corregidos para ENVASES
    
    Formatos pdfplumber:
    - Con descuento: 001124 MH CLASICA BARRIL 30L 5 108,8 50 21 272,00
      (CODIGO DESC UDS PRECIO DTO IVA IMPORTE)
    - Sin descuento: URG SERVICIO URGENTE 1 5 21 5,00
      (CODIGO DESC UDS PRECIO IVA IMPORTE)
    - Envase lit: CE990002 ENVASE 30 lit. 1 30 21 30,00
    - Envase ALH: CE990012 ENVASE 1/3 ALH -6 2,7 21 -16,20
    
    Formatos pypdf (IVA pegado a precio):
    ...
    # Patrón 1: Productos CON descuento (6 dígitos) - pdfplumber
    # 001124 MH CLASICA BARRIL 30L 5 108,8 50 21 272,00
    patron_pdfplumber_dto = re.compile(
    ...
    # Patrón 1b: Productos CE1xxx CON descuento - pdfplumber (1T25)
    # CE1393 ALH ESP 1/5 RET 30UDS 8 22,89 50 21 91,56
    # CE1384 VERMUT BENDITO BOX 20L 1 113,7 22,5 21 88,12
    patron_pdfplumber_ce1 = re.compile(
        r'^(CE\d{4})\s+'                 # Codigo CE + 4 dígitos
        r'([A-Z][A-Z0-9\s\'/\-\.]+?)\s+' # Descripcion
        r'(-?\d+)\s+'                    # Unidades
        r'(\d+(?:[,\.]\d+)?)\s+'         # Precio
        r'(\d+(?:[,\.]\d+)?)\s+'         # Descuento (puede tener decimales: 22,5)
        r'(\d{1,2})\s+'                  # IVA %
        r'(-?\d+[,\.]\d{2})',            # Importe
        re.MULTILINE
    )
    
    # Patrón 1c: Productos 6 dígitos SIN descuento - pdfplumber
    # 001229 SOUSAS GAS MEDIO RT 20 UDS 1 9,49 10 9,49
    patron_pdfplumber_sin_dto = re.compile(
        r'^(\d{6})\s+'                   # Codigo 6 dígitos
        r'([A-Z][A-Z0-9\s\'/\-\.]+?)\s+' # Descripcion
        r'(-?\d+)\s+'                    # Unidades
        r'(\d+[,\.]\d{2})\s+'            # Precio (con decimales para distinguir)
        r'(\d{1,2})\s+'                  # IVA (sin descuento intermedio)
        r'(\d+[,\.]\d{2})$',             # Importe (final de línea)
        re.MULTILINE
    )
    
    # Patrón 2: Productos cortos SIN descuento (URG, etc.) - pdfplumber
    # URG SERVICIO URGENTE 1 5 21 5,00
    patron_pdfplumber_corto = re.compile(
        r'^([A-Z]{2,4})\s+'              # Codigo corto
        r'([A-Z][A-Z0-9\s]+?)\s+'        # Descripcion
        r'(-?\d+)\s+'                    # Unidades
        r'(\d+(?:[,\.]\d+)?)\s+'         # Precio
        r'(\d{1,2})\s+'                  # IVA
    ...
    # Patrón 3: ENVASES tipo lit. - pdfplumber
    # CE990002 ENVASE 30 lit. 1 30 21 30,00
    patron_pdfplumber_envase_lit = re.compile(
        r'^(CE99\d{4})\s+'               # Codigo CE99xxxx
        r'(ENVASE.+?)\s+'                # Descripcion ENVASE + cualquier cosa
        r'(-?\d+)\s+'                    # Unidades
        r'(\d+)\s+'                      # Precio
        r'(\d+)\s+'                      # IVA
        r'(-?\d+[,\.]\d{2})',            # Importe
    ...
    # Patrón 4: ENVASES tipo ALH - pdfplumber
    # CE990012 ENVASE 1/3 ALH -6 2,7 21 -16,20
    patron_pdfplumber_envase_alh = re.compile(
        r'^(CE99\d{4})\s+'               # Codigo
        r'(ENVASE\s+\d/\d\s+ALH)\s+'     # ENVASE 1/3 ALH
        r'(-?\d+)\s+'                    # Unidades
        r'(\d+[,\.]\d+)\s+'              # Precio (con decimales: 2,7)
        r'(\d{1,2})\s+'                  # IVA
        r'(-?\d+[,\.]\d{2})',            # Importe
        re.MULTILINE
    )
    
    # ============== PATRONES PYPDF (fallback) ==============
    
    # Patrón 5: Productos CON descuento - pypdf (IVA pegado)
    # 001124 MH CLASICA BARRIL 30L 5 21108,8 50 272,00
    patron_pypdf_dto = re.compile(
    ...
    # Patrón 6: Envases - pypdf (IVA pegado)
    # CE990002 ENVASE 30 lit. -5 2130 -150,00
    patron_pypdf_envase = re.compile(
        r'^(CE99\d{4})\s+'               # Codigo CE99xxxx
    ...
    # ============== EXTRACCIÓN PDFPLUMBER (prioridad) ==============
    
    # 1. Productos con descuento (pdfplumber) - 6 dígitos
    for m in patron_pdfplumber_dto.finditer(texto):
        codigo, desc, uds, precio, dto, iva, importe = m.groups()
        key = (codigo, float(importe.replace(',', '.')))
        if key not in codigos_capturados:
    ...
    # 1b. Productos CE1xxx con descuento (pdfplumber) - 1T25
    for m in patron_pdfplumber_ce1.finditer(texto):
        codigo, desc, uds, precio, dto, iva, importe = m.groups()
        key = (codigo, float(importe.replace(',', '.')))
        if key not in codigos_capturados:
            codigos_capturados.add(key)
    ...
    # 1c. Productos 6 dígitos SIN descuento (pdfplumber)
    for m in patron_pdfplumber_sin_dto.finditer(texto):
        codigo, desc, uds, precio, iva, importe = m.groups()
        importe_f = float(importe.replace(',', '.'))
        # Ignorar líneas de 0,00 (envases/retornos sin cargo)
        if importe_f == 0.0:
            continue
        key = (codigo, importe_f)
        if key not in codigos_capturados:
            codigos_capturados.add(key)
            lineas.append({
                'codigo': codigo,
    ...
    # 2. Productos cortos (pdfplumber)
    for m in patron_pdfplumber_corto.finditer(texto):
        codigo, desc, uds, precio, iva, importe = m.groups()
        key = (codigo, float(importe.replace(',', '.')))
        if key not in codigos_capturados:
            codigos_capturados.add(key)
    ...
    # 3. Envases tipo lit. (pdfplumber)
    for m in patron_pdfplumber_envase_lit.finditer(texto):
        codigo, desc, uds, precio, iva, importe = m.groups()
        key = (codigo, float(importe.replace(',', '.')))
        if key not in codigos_capturados:
            codigos_capturados.add(key)
    ...
    # 4. Envases tipo ALH (pdfplumber)
    for m in patron_pdfplumber_envase_alh.finditer(texto):
        codigo, desc, uds, precio, iva, importe = m.groups()
        key = (codigo, float(importe.replace(',', '.')))
        if key not in codigos_capturados:
            codigos_capturados.add(key)
    ...
    # ============== FALLBACK PYPDF (si no hay resultados) ==============
    
    if not lineas:
        # 5. Productos con descuento (pypdf)
        for m in patron_pypdf_dto.finditer(texto):
            codigo, desc, uds, iva, precio, dto, importe = m.groups()
            desc_limpia = desc.strip()
            if 'ENVASE' in desc_limpia.upper() or 'REAJUSTE' in desc_limpia.upper():
                continue
            key = (codigo, float(importe.replace(',', '.')))
            if key not in codigos_capturados:
                codigos_capturados.add(key)
        ...
        # 6. Envases (pypdf)
        for m in patron_pypdf_envase.finditer(texto):
            codigo, desc, uds, iva, precio, importe = m.groups()
            lineas.append({
                'codigo': codigo,
                'articulo': desc.strip(),
                'iva': int(iva),
...
def extraer_lineas_bernal(texto: str) -> List[Dict]:
    """Extrae líneas de facturas JAMONES Y EMBUTIDOS BERNAL - v3.44 con pdfplumber.
    
    Soporta DOS formatos:
    
    1. pdfplumber (código al principio):
       EM-MINERO MORCÓN ACHORIZADO DE 0,00 2,090 12,7300 0,00 10,00 26,606
       Formato: CODIGO DESCRIPCION CSEC UDS PRECIO %DES %IVA IMPORTE
    
    2. pypdf (código pegado al final):
       Lotes: 20242; MORCÓN ACHORIZADO DE BELLOTA... 26,606EM-MINERO
       La descripción puede estar en múltiples líneas.
    """
    lineas = []
    
    # PATRÓN 1: pdfplumber (código al principio de línea) - PRIORITARIO
    # Formato: CODIGO DESCRIPCION CSEC UDS PRECIO %DES %IVA IMPORTE
    patron_pdfplumber = re.compile(
        r'^([A-Z]{1,3}-[A-Z]+)\s+'          # Código (EM-MINERO, LO-JABELL, P-PORTES)
        r'([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑa-záéíóúñ0-9\s%]+?)\s+'  # Descripción
        r'([\d,]+)\s+'                       # C.Sec
        r'([\d,]+)\s+'                       # Unidades  
        r'([\d,]+)\s+'                       # Precio
        r'([\d,]+)\s+'                       # %Des
    ...
    for match in patron_pdfplumber.finditer(texto):
        codigo, desc, csec, uds, precio, dto, iva_str, importe = match.groups()
        desc_limpia = ' '.join(desc.split()).strip()
        
        # Ignorar cabeceras o descripciones muy cortas
        if len(desc_limpia) < 3 or 'Producto' in desc_limpia or 'C.Sec' in desc_limpia:
            continue
    ...
    # PATRÓN 2: pypdf (código pegado al final) - FALLBACK
...
def extraer_lineas_serrin(texto: str) -> List[Dict]:
    """Extrae líneas de facturas SERRÍN NO CHAN - v3.44 con soporte pdfplumber.
    ...
    1. pdfplumber (tabular): LOD Vermú rojo botella 75cl 12 7,93 21,00% 19,98 € 95,16 € 115,14 €
       - Líneas completas con todos los datos
    
    2. pypdf (fragmentado): Artículos, números y códigos en bloques separados
       - Requiere correlación por posición
    
    IVA: Explícito y variable (4% galletas ECO, 10% normales, 21% vermú)
    """
    lineas = []
    
    # ESTRATEGIA 1: pdfplumber (formato tabular completo) - PRIORITARIO
    # Formato: CODIGO ARTICULO CANTIDAD PRECIO IVA% IVA€ BASE€ TOTAL€
    # Ejemplo: LOD Vermú rojo botella 75cl 12 7,93 21,00% 19,98 € 95,16 € 115,14 €
    patron_pdfplumber = re.compile(
        r'^([A-Z]{2,5})\s+'                              # Código (2-5 letras)
        r'([A-Za-záéíóúñÁÉÍÓÚÑ][A-Za-z0-9áéíóúñÁÉÍÓÚÑ\s]+?)\s+'  # Artículo
        r'(\d{1,3})\s+'                                  # Cantidad
        r'(\d+[,\.]\d{2})\s+'                            # Precio unitario
        r'(\d{1,2})[,\.]00\s*%\s+'                       # IVA %
        r'(\d+[,\.]\d{2})\s*€\s+'                        # Importe IVA
        r'(\d+[,\.]\d{2})\s*€',                          # Base imponible
        re.MULTILINE
    )
    
    for match in patron_pdfplumber.finditer(texto):
        codigo, articulo, cantidad, precio, iva_pct, iva_imp, base = match.groups()
        articulo_limpio = articulo.strip()
        
        # Ignorar líneas de totales/headers
        if any(x in articulo_limpio.lower() for x in ['albarán', 'i.v.a', 'sobre', 'total']):
            continue
    ...
    # ESTRATEGIA 2: pypdf (formato fragmentado) - FALLBACK
    ...
    # 2. Datos numéricos: "7,93 19,98 € 95,16 € 115,14 €"
    ...
    # Si no encontró nada, texto pegado
...
def procesar_factura(ruta: Path, indice: Dict, carpeta_yaml: Path = None) -> Factura:
    ...
    # v3.27 - LA MOLIENDA VERDE (antes del genérico)
        ...
        # v3.44 - Extraer total específico para CERES (soporta negativos)
        # pdfplumber: "Importe TOTAL ........ -22,87"
        # pypdf: "Importe TOTAL ........\nNIF: B83478669\n588,06\nCLA:"
        if factura.total is None:
            # Primero intentar formato pdfplumber (puntos + número en misma línea)
            total_match = re.search(r'Importe\s*TOTAL\s*\.+\s*(-?\d+[.,]\d{2})', texto, re.IGNORECASE)
            if total_match:
                factura.total = float(total_match.group(1).replace(',', '.'))
            else:
                # Fallback: formato pypdf (número en línea separada)
