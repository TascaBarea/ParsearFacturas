...
def extraer_lineas_licores_madrueño(texto: str) -> List[Dict]:
    ...
    # Formato: 4291 12A NTONIO MONTERO A lbariño 71,405,95
    # El texto tiene espacios raros, hay que limpiarlo
    # Patrón: CODIGO UNIDADES+ARTICULO IMPORTE,PRECIO
    
    patron = re.compile(
        r'^(\d{2,4})\s+'  # Código 2-4 dígitos
        r'(\d{1,3})'  # Unidades
        r'([A-Z][A-Z\s\´\'\-]+?)\s*'  # Artículo (empieza con mayúscula)
        r'(\d{1,3}[.,]\d{2})'  # Importe
    ...
    for match in patron.finditer(texto):
        codigo, uds, articulo, importe, precio = match.groups()
        # Limpiar artículo (quitar espacios extra)
...
def extraer_lineas_sabores_paterna(texto: str) -> List[Dict]:
    ...
    # Formato: CHICHARRON ESPECIAL 0,007 9,010 16,8010,0 151,37
    # DESCRIPCION UNIDADES PESO PRECIO IVA IMPORTE
    patron = re.compile(
        r'^(\d{2}-\d{2}-\d{2})?'  # Fecha opcional
    ...
    for match in patron.finditer(texto):
        grupos = match.groups()
        articulo = grupos[1].strip() if grupos[1] else ""
        iva = int(grupos[4]) if grupos[4] else 10
        importe = grupos[5] if grupos[5] else "0"
...
def extraer_lineas_ceres(texto: str) -> List[Dict]:
    ...
    # Formato: CE1384 VERMUT BENDITO BOX 20L -7 113,7 22,5 21 -616,82
    patron = re.compile(
        r'^([A-Z]{2}\d+)\s+'  # Código (CE1384)
    ...
    for match in patron.finditer(texto):
        codigo, articulo, uds, precio, dto, iva, importe = match.groups()
        articulo_limpio = articulo.strip()
